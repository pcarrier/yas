...
one `yas.v1` WebSocket at `/edge` and adapts it to one fixed home YAS socket.
...
enum Transport {
    ...
    Duplex(tokio::io::DuplexStream),  // SSH, WebSocket, WebTransport, WebRTC
}
```
...
datagram use their reliable encoding on this transport.
...
`yas proxy-daemon` eliminates that latency by maintaining a pool of pre-warmed,
already-authenticated connections to each upstream. A client gets a live
connection from the pool immediately, and the pool refills in the background.
The home-server Relay service owns its routes and calls the same connector
implementations directly; routing never moves into the browser-facing edge.
...
graph LR
    ...
    P -->|"pool[ssh:rabbit]\nssh bridge"| SR["yas server (rabbit)"]
    P -->|"pool[wss://prod/edge]\nWebSocket"| GW["YAS edge (prod)"]
```
...
- Each pool pre-connects `YAS_PROXY_POOL` (default: 4) idle connections.
...
`YAS_PROXY_IDLE=<seconds>` causes the proxy to exit when all pools have been idle for that many seconds. A watcher task checks every 5 seconds, ignoring pools with active clients. By default no idle timeout is set and the daemon runs indefinitely.
...
| Remote CLI via WebTransport    | yas proxy-daemon → WebTransport         | `YAS_PROXY=0` → direct WebTransport  |
| Browser → edge                 | WebSocket `/edge` → fixed home socket   | WSS/TLS required beyond loopback     |
