...
- **Layout content has live and persisted identities.** The resolved in-memory
  ...
  terminals. Workspace sessions persist stable `terminal:`, `surface:`, and
...
cycle-focus, persist to the workspace session, and appear in presets exactly like
...
`gitlog:` helpers to `js/core/src/layout/tree.ts` and teach Workspace session
...
- _Mechanism:_ a `kind` field on `LayoutLeaf` (`js/core/src/layout/dsl.ts`); an
  ...
  branch) and the workspace session persists; editor/diff keep landing only through
