...
PipeWire, a portal frontend, and the `xdg-desktop` extension to keep applications up.
...
the shipped `xdg-desktop` extension, an ordinary desktop entry, and a wrapper
...
The `xdg-desktop` extension supervises **desktop entries**, not commands: its
...
([`extensions/xdg-desktop/src/main.rs`](../extensions/xdg-desktop/src/main.rs)).
...
The parser
([`extensions/xdg-desktop/src/desktop_entry.rs`](../extensions/xdg-desktop/src/desktop_entry.rs))
...
- The id is the **basename without `.desktop`** — `yas-user-manager` here — and
  that is what `@xdg-desktop enable` takes.
...
```bash
yas ext run --persist --restart always xdg-desktop xdg-desktop.wasm
yas @xdg-desktop list                      # yas-user-manager  no  -  User service manager
yas @xdg-desktop enable yas-user-manager
yas @xdg-desktop status yas-user-manager
```
...
intent survive a restart. `enable` is durable — it is stored under `ext/xdg-desktop/app/<id>` in the
...
([`extensions/xdg-desktop/src/supervisor.rs`](../extensions/xdg-desktop/src/supervisor.rs))
apply to the manager like any other application:

- **A clean exit is never retried.** `systemctl --user exit` (or the `TERM` the
  extension sends on `@xdg-desktop disable`) exits 0, so the manager stops and stays
  stopped — no respawn loop. Bring it back with `@xdg-desktop enable`.
...
- Children are `SPAWN_DETACHABLE`: they survive a restart of `xdg-desktop`
...
Measured on systemd 261, yas `main`, `xdg-desktop.wasm`
