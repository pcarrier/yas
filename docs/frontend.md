...
- Surface cursor metadata drives the canvas CSS cursor. Withdrawing that metadata, removing a surface, or resetting the store restores the default cursor on mounted canvases. Absolute pointer motion over a hidden guest cursor forces a fresh Wayland leave/re-enter unless the client holds a real pointer lock. This covers both ordinary fullscreen-video idle hides and players that retain hidden state while the pointer enters, while the fresh enter serial makes the client restate the actual cursor under the pointer instead of stranding the viewer on a synthetic default arrow. The browser also provides an immediate local recovery when the hide followed an idle interval, when hidden state was cached across a guest reload, or when another viewer took pointer ownership. Pointer-lock applications and games keep their intentional hidden cursor. A non-empty mirrored pointer proves another viewer took the shared Wayland pointer, so the prior viewer also drops its local ownership claim. Same-surface viewer handoffs likewise produce a Wayland leave/re-enter, giving the new owner a fresh cursor serial and invalidating late cursor requests from the old position.
