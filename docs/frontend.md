...
The durable UI around those live processes is a **workspace session**. The
...
presence bit. The canonical browser URL is only `#session=<id>`; layout and
panel mutations use bounded CAS retries against the backend record. See
[Backend workspace sessions](design/workspace-sessions.md) for the record and
lifecycle contract.
