...
The browser stores each durable workspace session as one strict versioned
...
and side-panel state. The URL is consequently only `#session=<uuid>`; the UUID
...
- **Further UI-state consumers** — roots and workspace sessions prove the
...
4. Workspace sessions, device attachment records, roots, and editor state use
