...
canonical payload replays its Result without applying the backend mutation or
advancing the Surface revision again. A later successful same-kind mutation of
the same Surface supersedes that slot; a rejected or failed successor does not.
The replay horizon of a superseded ID has ended: the client MUST reconcile the
authoritative WATCH catalogue state and MUST NOT retry that old ID.
...
latency target, and decoder capacity for one client mount. CONFIGURE_VIEW
replaces the supplied values without resubscription. CAPTURE returns an inline
image up to 32 KiB or a byte Transfer carrying the selected PNG or AVIF object.
...
immediately and timeout retires them if a client disappears.
...
| Browser authentication and transport adaptation                    | Fixed-home `yas.v1` WebSocket edge; native read-only `yas.v1` WebRTC share                             |
...
- native workspace sessions whose URL contains only the durable backend session
