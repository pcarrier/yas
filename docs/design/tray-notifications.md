...
The native menu tree retains an explicit root with an opaque handle. The
browser presentation omits that root and maps its children to parent ID `0`.
Other parent IDs are mapped to the same presentation IDs as their submenu
nodes, including when a node's action handle differs from its node handle.
Menu selections retain the original action handle and menu revision.
