...
- an ordered set of active Relay route names (the local home route is
  implicit);
- layout name and structured tree, pane assignments, focused pane, and main focused content;
- optional `soloedPaneId`, restoring the pane filling the workspace after reload
  or reattachment; null or absent shows all panes. A tree change clears solo
  state because pane IDs are positional paths;
- optional `parkedPlacements`, mapping stable resource references to their tiled
  or floating mode, floating frame, and reference viewport; restoring a view
  consumes its entry;
...
Layouts store `{name, root}` directly, with no text-language representation or
migration. The shared tree validator admits at most 2,048 panes, 64 levels, and
256 KiB, and checks container kinds, child weights, pane settings, and floating
frames before a session record or device-local layout is accepted.
...
While the selected record is loading, the browser renders session controls
and a loading indicator without mounting panes or applying a device-local
fallback layout. The protocol workspace stays connected throughout. Once the
binding arrives, its saved layout is the first pane tree mounted. An empty
attachment list or a load failure keeps the session controls available without
opening an unbound workspace; unmanaged embeds still open immediately.
