...
closing the reliable YAS session. The family codecs and endpoint model have
landed; the complete composite WebTransport/WebRTC path and its loss, ordering,
congestion, MTU, and fallback matrix remain an explicit release gate in
[yas.md](yas.md#open-release-blockers).
