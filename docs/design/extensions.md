...
Replacement-form `DEPLOY` may carry required extension tag
`DEPLOY_PRESERVE_ARGV_TAG` with an empty value and an empty `argv`. The server
loads the arguments of the exact expected identity and revision before making
the ordinary compare-and-swap replacement. This lets registry installers
update immutable module bytes without exposing raw persistent arguments in
State or erasing settings the installer cannot observe.
...
yas ext manage [--from REGISTRY_URL]
yas ext list
...
`manage` opens an inline picker for installing, updating, and uninstalling
persistent extensions. Space or a click cycles a row's action; `d` selects
uninstall directly. Enter applies the selected changes. Uninstall disables the
definition and waits for `STOPPED` or `BLOCKED` before removing it, including
definitions absent from the selected registry. Concurrent definition changes
abort removal so the user can review the new state.
