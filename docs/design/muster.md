...
**Chosen over three things that exist.** `xdg-desktop` supervises desktop entries
...
migration into an outage. An initial run still gates dependents, and stopping an
in-progress run still cascades immediately. Stopping a completed `oneshot` is a
no-op: there is no process to stop, and its successful result remains ready.
...
The server requires that build result. Its WebTransport bind and advertised
port are explicitly tied to the allocated edge port as well; otherwise an
ambient service configuration can make every worktree contend for the default
`:3264` UDP listener:
...
```json
{
  ...
  "env": {
    ...
    "YAS_WEBTRANSPORT_ADDR": "127.0.0.1:${PORTS+1}",
    "YAS_WEBTRANSPORT_PUBLIC_PORT": "${PORTS+1}",
    "YAS_CORS": "*",
    "YAS_FONT_EXPORT": "1"
  },
...
`ext/xdg-desktop/`. The durable tail is why `@muster log` still says why something
...
posture as `yas.xdg-desktop.v1`, and as being able to open a terminal at all.
...
whole extension is **410 KB, 137 KB brotli**, against `xdg-desktop`'s 187 KB / 68 KB
