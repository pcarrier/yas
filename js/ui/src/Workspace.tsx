...
function WorkspaceScreen(props: {
  ...
  const initialLayout = (() => {
    ...
    if (!stored) return initialSessionWorkspace ? null : loadActiveLayout();
    try {
  ...
  // A workspace session's main tab reference resolves asynchronously against
  ...
  createEffect(() => {
    ...
    // Web panes are registered like every other tab: a workspace session keeps
  ...
  const offScreenSurfaces = createMemo<YasSurface[]>((prev) => {
  ...
  const offScreenSessions = createMemo<YasSession[]>((prev) => {
    const al = activeLayout();
    const la = layoutAssignments();
    const sess = sessions();
    if (al) {
   ...
   * two-pane one. With a layout up, LayoutContainer binds the same two keys and
   * splits the focused pane instead — this path only ever has one pane to
   * divide. The occupant carries into the first pane on its own: LayoutContainer
   * seeds a fresh layout from the focused session outwards.
   */
  function splitFocusedView(direction: "horizontal" | "vertical") {
    if (activeLayout()) return;
    const dsl = direction === "horizontal" ? "line(_, _)" : "col(_, _)";
  ...
  /** Leave the layout without dropping the focused surface back into the preview
   *  panel. Terminal focus already lives in YasWorkspace, but surface focus
   *  is derived from the layout assignment while the container is mounted. Move
   *  it into the single-view focus slot before clearing assignments so the new
   *  foreground view mounts, offers its full size, and survives the old layout
   *  view's resize withdrawal. */
  function exitLayout() {
    if (inLayout()) {
      const surface = layoutFocusedSurface();
      ...
      // *before* entering the layout — so leaving a layout of terminals demoted that
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* In a layout each pane carries its own ✕ (LayoutContainer), so
                  this one would be a second, ambiguous control over whichever
                  pane happens to be focused. */}
              <Show
                when={!(inLayout() && activeLayout()) && mainViewClosable()}
              >
                <PaneTools
                  theme={theme()}
                  scale={chromeScale()}
              ...
              >
                {(al) => (
                  <LayoutContainer
                    ...
                    storedAssignments={initialSessionWorkspace?.assignments}
                    storedFocusedPaneId={initialSessionWorkspace?.focusedPaneId}
        ...
        <Show when={overlay() === "remotes" && shellCapabilities().remotes}>
          {(_) => (
            <RemotesOverlay
              ...
              onSetSessionActive={(name, active) =>
                props.workspaceSession?.setRemoteActive(name, active)
              }
              onReconnect={(name) => workspace.reconnectConnection(name)}
              stored={homeStoredRemotes()}
              onAddRemote={(name, uri) =>
                homeConnectionId()
                  ? addStoredRemote(workspace, homeConnectionId()!, name, uri)
                  : undefined
              }
              onRemoveRemote={(name) =>
                homeConnectionId()
      ...
      header={() => (
        <>
          ...
          >
            ...
            {props.surface.title ||
              props.surface.appId ||
              `Surface ${props.displayHandle ?? props.surface.surfaceId}`}
          </span>
        </>
      )}
      body={() => (
