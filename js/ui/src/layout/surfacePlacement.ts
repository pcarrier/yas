...
/**
 * Whether a first-seen surface should enter the managed layout.
 *
 * The first authoritative catalogue after restoring a saved layout also
 * contains windows that are deliberately absent from its assignments: those
 * are parked, not newly opened. Once that connection's initial catalogue has
 * completed, every newly observed toplevel is a real arrival and is placed.
 * An explicitly restored assignment still proceeds through the resolver
 * during the initial catalogue.
 */
export function shouldPlaceObservedSurface(options: {
  restoringSavedLayout: boolean;
  initialCataloguesComplete: ReadonlySet<string>;
  connectionId: string;
  explicitlyRestored: boolean;
}): boolean {
  return (
    !options.restoringSavedLayout ||
    options.explicitlyRestored ||
    options.initialCataloguesComplete.has(options.connectionId)
  );
}
