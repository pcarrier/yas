...
export type PanePlacement = WorkspacePanePlacement;
...
function appendToTiledBase(base: LayoutNode, node: LayoutNode): LayoutNode {
  if (base.type === "split" && base.direction !== "workspace") {
    return {
      ...base,
      children: [
        ...base.children,
...
/** Top-level frame index for a floating pane, including nested containers. */
...
/** Placement to restore when a pane's assignment is parked. */
export function panePlacement(root: LayoutNode, paneId: string): PanePlacement {
  const frameIndex = floatingFrameIndex(root, paneId);
  if (frameIndex == null || root.type !== "split") return { mode: "tiled" };
  return {
    mode: "floating",
    rect: clampRect(root.children[frameIndex].rect ?? cascadeRect(frameIndex)),
  };
}
...
/** Add a new independent floating view above the tiled workspace. */
