  ...
  const prune = (node: LayoutNode, isRoot: boolean): LayoutNode | null => {
    ...
    if (children.length === 1) {
      if (
        isRoot &&
        !launcherOnly &&
        node.direction === "workspace" &&
        children[0].rect != null
      ) {
...
export function removePaneFromLayout(
  ...
  const removeAt = (
    ...
    if (children.length === 0) return null;
    // Preserve the frame of a workspace containing one floating window.
    // Nested one-child splits and tiled roots collapse normally.
    if (children.length === 1) {
      if (
        current === node &&
        current.direction === "workspace" &&
        children[0].rect != null
      ) {
