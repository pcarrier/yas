...
const XDG_DESKTOP = "yas.xdg-desktop.v1";
...
describe("following channel presence", () => {
  it("reports what is served now and every later change", async () => {
    const server = watchingHost([XDG_DESKTOP]);
    const seen: string[][] = [];
    ...
    expect(seen).toEqual([[XDG_DESKTOP]]);

    server.publish([XDG_DESKTOP, SYSTEMD]);
    server.publish([SYSTEMD]);
    expect(seen).toEqual([[XDG_DESKTOP], [XDG_DESKTOP, SYSTEMD], [SYSTEMD]]);
  ...
  it("treats a refused connect as the answer, not a failure", async () => {
    ...
    await expect(
      probeChannelNames(server.host, [XDG_DESKTOP, SYSTEMD]),
  ...
  it("asks for both names in one watch", async () => {
    ...
    await followChannelNames(server.host, [XDG_DESKTOP, SYSTEMD], () => {});
    expect(watch).toHaveBeenCalledTimes(1);
    expect(watch.mock.calls[0]![0]).toEqual([XDG_DESKTOP, SYSTEMD]);
  });
});
