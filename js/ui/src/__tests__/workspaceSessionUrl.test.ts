...
describe("workspace URL", () => {
  it("reads canonical workspace IDs and legacy session links", () => {
    expect(workspaceSessionIdFromHash(`#workspace=${ID.toUpperCase()}`)).toBe(
      ID,
    );
    expect(workspaceSessionIdFromHash(`#debug&workspace=${ID}`)).toBe(ID);
    expect(workspaceSessionIdFromHash(`#session=${ID}`)).toBe(ID);
    expect(workspaceSessionIdFromHash("#workspace=not-a-workspace")).toBeNull();
    expect(normalizeWorkspaceSessionId(`${ID}x`)).toBeNull();
    expect(workspaceSessionRequestFromHash("#workspace=malformed")).toEqual({
      present: true,
      id: null,
    });
    expect(
      workspaceSessionRequestFromHash(`#session=malformed&workspace=${ID}`),
    ).toEqual({ present: true, id: ID });
  ...
  it("emits only the canonical workspace field", () => {
    expect(workspaceSessionHash(ID)).toBe(`workspace=${ID}`);
    ...
    ).toBe(`https://yas.example/ui#workspace=${ID}`);
  });

  it("copies only the canonical workspace URL", () => {
    expect(
      workspaceSessionShareUrl(
        { origin: "https://yas.example", pathname: "/console" } as Location,
        ID,
      ),
    ).toBe(`https://yas.example/console#workspace=${ID}`);
  });

  it("removes all passphrase fields while retaining other fields exactly", () => {
    expect(
      consumePassphraseFromHash(
        `#workspace=${ID}&psk=hello%20world&debug&psk=ignored`,
      ),
    ).toEqual({
      passphrase: "hello world",
      hash: `workspace=${ID}&debug`,
      found: true,
    });
  });

  it("does not rewrite a fragment without a delivered credential", () => {
    expect(consumePassphraseFromHash(`#workspace=${ID}`)).toEqual({
      passphrase: null,
      hash: `workspace=${ID}`,
      found: false,
    });
  ...
  it("records the workspace and leaves a clean address", () => {
    history.replaceState(
      null,
      "",
      `/app?ignored=1#workspace=${ID}&session=${ID}&old`,
    );
  ...
  it("never leaves a first-contact passphrase in the address", () => {
    history.replaceState(null, "", `/app#psk=secret&workspace=${ID}`);
    writeWorkspaceSessionUrl(ID, "replace");
    expect(location.hash).toBe("");
  });

  it("reads back the workspace this device attached", () => {
