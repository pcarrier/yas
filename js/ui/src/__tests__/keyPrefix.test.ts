...
describe("isPrefixChord", () => {
  it("is Ctrl+B on every platform", () => {
    ...
    expect(isPrefixChord(chord("B", { ctrlKey: true }), "MacIntel")).toBe(true);
    expect(
      isPrefixChord(chord("", { ctrlKey: true, code: "KeyB" }), "Linux x86_64"),
    ).toBe(true);
    expect(
      isPrefixChord(
        { ...chord("\u0002", { ctrlKey: true }), keyCode: 229 },
        "MacIntel",
      ),
    ).toBe(true);
    expect(
      isPrefixChord(
        { ...chord("Unidentified", { ctrlKey: true }), keyCode: 66 },
        "MacIntel",
      ),
    ).toBe(true);
  });
  ...
  it("also accepts Command+B on Apple platforms", () => {
    expect(isPrefixChord(chord("b", { metaKey: true }), "MacIntel")).toBe(true);
    expect(isPrefixChord(chord("b", { metaKey: true }), "iPad")).toBe(true);
    expect(isPrefixChord(chord("b", { metaKey: true }), "Linux x86_64")).toBe(
      false,
    );
  });
  ...
  it("rejects the prefix chord with any other modifier", () => {
    ...
    expect(
      isPrefixChord(chord("b", { ctrlKey: true, metaKey: true }), "MacIntel"),
    ).toBe(false);
...
describe("prefixToken", () => {
  ...
  it("gives the space bar a visible, bindable token", () => {
    expect(prefixToken(chord(" ", { code: "Space" }))).toBe("Space");
    expect(prefixToken(chord(" ", { code: "Space", shiftKey: true }))).toBe(
      "Shift+Space",
    );
  });

  it("keeps modifiers on named navigation keys", () => {
    expect(prefixToken(chord("ArrowLeft", { shiftKey: true }))).toBe(
      "Shift+ArrowLeft",
    );
    expect(prefixToken(chord("ArrowDown", { altKey: true }))).toBe(
      "Alt+ArrowDown",
    );
  });
...
describe("the map behind the prefix", () => {
  ...
  it("represents repeated bindings as one deliberate menu entry", () => {
    disarmPrefix();
    const menu = { token: "← ↑ ↓ →", group: "focus-direction" };
    const drops = [
      registerPrefixAction("ArrowLeft", () => {}, "Focus spatially", menu),
      registerPrefixAction("ArrowRight", () => {}, "Focus spatially", menu),
      registerPrefixAction("ArrowUp", () => {}, "Focus spatially", menu),
      registerPrefixAction("ArrowDown", () => {}, "Focus spatially", menu),
    ];

    expect(prefixBindings()).toEqual([
      { token: "← ↑ ↓ →", label: "Focus spatially" },
    ]);
    expect(prefixActionTokens()).toEqual([
      "ArrowDown",
      "ArrowLeft",
      "ArrowRight",
      "ArrowUp",
