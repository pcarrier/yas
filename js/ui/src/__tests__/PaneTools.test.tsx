...
describe("PaneTools status-bar actions", () => {
  ...
  it("keeps pane focus and toggles float/tile and zoom/restore both ways", () => {
    ...
    expect(floatingButton.textContent).toContain("Tile");
    floatingButton.click();
    expect(floating()).toBe(false);
    expect(action("floating").textContent).toContain("Float");
    ...
    expect(soloButton.textContent).toContain("Restore");
    soloButton.click();
    expect(solo()).toBe(false);
    expect(action("solo").textContent).toContain("Zoom");
  });
