...
import {
  openXdgDesktop,
  XDG_DESKTOP_ARTWORK_READ_CREDIT,
  XDG_DESKTOP_ARTWORK_MAX_BYTES,
  XDG_DESKTOP_ARTWORK_MAX_ENTRIES,
  XDG_DESKTOP_ICON_QUEUE_MAX_BYTES,
  XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES,
  XDG_DESKTOP_ICON_REQUEST_MAX_ENTRIES,
  XDG_DESKTOP_MAX_ID_CHARS,
  XDG_DESKTOP_MAX_APPS,
  XDG_DESKTOP_MAX_CATALOG_ENTRIES,
  XDG_DESKTOP_MAX_NAME_CHARS,
  XDG_DESKTOP_MAX_STATE_BYTES,
  XdgDesktopMirror,
} from "../xdgDesktop";
...
describe("XdgDesktopMirror", () => {
  ...
  it("bounds hostile state by rows and retained bytes", () => {
    ...
    mirror.apply(
      state(
        Array.from({ length: XDG_DESKTOP_MAX_APPS + 50 }, (_, index) => ({
          id: `app-${index}`,
          name: "a".repeat(XDG_DESKTOP_MAX_NAME_CHARS * 2),
        })),
        Array.from(
          { length: XDG_DESKTOP_MAX_CATALOG_ENTRIES + 50 },
          (_, index) => ({
            id: `catalog-${index}`,
            name: "c".repeat(XDG_DESKTOP_MAX_NAME_CHARS * 2),
          }),
        ),
      ),
    );
    ...
    expect(stats.apps).toBeLessThanOrEqual(XDG_DESKTOP_MAX_APPS);
    expect(stats.catalog).toBeLessThanOrEqual(XDG_DESKTOP_MAX_CATALOG_ENTRIES);
    expect(stats.bytes).toBeLessThanOrEqual(XDG_DESKTOP_MAX_STATE_BYTES);
    expect(
      mirror.apps.every((app) => app.name.length <= XDG_DESKTOP_MAX_NAME_CHARS),
    ).toBe(true);
    expect(
      mirror.catalog.every(
        (entry) => entry.name.length <= XDG_DESKTOP_MAX_NAME_CHARS,
      ),
    ).toBe(true);
  });
  ...
  it("LRU-bounds artwork and revokes evicted and reconciled blob URLs", () => {
    ...
    const ids = Array.from(
      { length: XDG_DESKTOP_ARTWORK_MAX_ENTRIES + 8 },
      (_, index) => `app-${index}`,
    );
    mirror.apply(
      state(
    ...
    expect(mirror.cacheStats().entries).toBe(XDG_DESKTOP_ARTWORK_MAX_ENTRIES);
...
describe("openXdgDesktop", () => {
  const fakeConnection = (initiallyWritable = true) => {
    ...
    } & Parameters<typeof openXdgDesktop>[0];
  };
  ...
  it("keeps the first icon batch queued until channel credit arrives", async () => {
    ...
    try {
      ...
      const session = await openXdgDesktop(connection);
  ...
  it("bounds queued peer ids and reconciles them against fresh state", async () => {
    ...
    try {
      ...
      const session = await openXdgDesktop(connection);
      const ids = Array.from(
        { length: XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES + 100 },
        (_, index) => `app-${index}`,
      );
      connection.inbound.deliver?.(
        state(
      ...
      ).toHaveLength(XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES);
  ...
  it("keeps the full authoritative catalog in the asked-id window", async () => {
    ...
    try {
      ...
      const session = await openXdgDesktop(connection);
      const ids = Array.from(
        { length: XDG_DESKTOP_ICON_REQUEST_MAX_ENTRIES + 100 },
      ...
      for (
        let at = 0;
        at < ids.length;
        at += XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES
      ) {
        session.requestIcons(
          ids.slice(at, at + XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES),
        );
        vi.advanceTimersByTime(200);
      }
  ...
  it("applies the queued byte cap to maximum-length peer ids", async () => {
    ...
    try {
      ...
      const session = await openXdgDesktop(connection);
      const ids = Array.from(
        { length: XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES },
        (_, index) =>
          `${index}-`.padEnd(XDG_DESKTOP_MAX_ID_CHARS, String(index % 10)),
      );
      connection.inbound.deliver?.(
        state(
          [],
      ...
      expect(sent.length).toBeLessThan(XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES);
      expect(
        sent.reduce((bytes, id) => bytes + 32 + id.length * 2, 0),
      ).toBeLessThanOrEqual(XDG_DESKTOP_ICON_QUEUE_MAX_BYTES);
      session.close();
    } finally {
      vi.useRealTimers();
    }
  ...
  it("reads artwork on the next task when an icon path arrives", async () => {
    ...
    try {
      ...
      > & {
        readFiles: NonNullable<
          Parameters<typeof openXdgDesktop>[0]["readFiles"]
        >;
      };
  ...
  it("uses one bounded receive window for a full visible icon shelf", async () => {
    ...
    try {
      ...
      for (const call of readFiles.mock.calls) {
        expect(call[1]).toEqual({ maxBytes: XDG_DESKTOP_ARTWORK_READ_CREDIT });
      }
      session.close();
    } finally {
      create.mockRestore();
