...
import { disarmPrefix, prefixArmed, registerPrefixAction } from "../keyPrefix";
...
describe("hasFocusedExitedTerminal", () => {
  ...
  it("rejects running and background terminals", () => {
    ...
    expect(
      hasFocusedExitedTerminal(restartFocus("exited", "tile:editor")),
    ).toBe(false);
  });
});

describe("mobile one-shot Ctrl prefix", () => {
  it("arms Ctrl+B before the terminal consumes the toolbar modifier", () => {
    disarmPrefix();
    let active = true;
    const surface = {
      get ctrlModifier() {
      ...
      setCtrlModifier(next: boolean) {
        active = next;
      },
    };
    let opened = 0;
    const unbind = registerPrefixAction("k", () => opened++);
