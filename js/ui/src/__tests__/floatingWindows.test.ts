...
describe("floating window chrome", () => {
  ...
  it("remembers whether a parked pane was tiled or floating", () => {
    ...
    expect(panePlacement(mixed!.root, "0.1")).toEqual({ mode: "tiled" });
    expect(panePlacement(mixed!.root, "1")).toEqual({
      mode: "floating",
      rect: { x: 18, y: 12, width: 58, height: 62 },
    });
  });
    ...
    const root = {
      ...
      children: [
        ...
        },
        {
          node: { type: "leaf" },
          weight: 1,
          rect: { x: 24, y: 20, width: 55, height: 60 },
        },
      ],
    } as LayoutNode;
    const mixed = togglePaneFloating(
