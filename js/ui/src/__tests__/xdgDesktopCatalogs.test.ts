  ...
  const connection = {
    watchChannelNames(
      ...
      watches.push({
        ...
        publish(served) {
          present.clear();
          if (served) present.add(XDG_DESKTOP_CHANNEL);
          onNames(present);
        },
        stop,
      });
    ...
    connectChannel(
      ...
      opens.push({ options, result, close, send, channel });
...
afterEach(async () => {
  dropXdgDesktopCatalog(CONNECTION_ID);
  vi.useRealTimers();
  await settle();
});

describe("XDG desktop catalog channel lifecycle", () => {
  it("requests the complete icon shelf as soon as the catalog arrives", async () => {
    vi.useFakeTimers();
    const remote = fakeRemote();
    ensureXdgDesktopCatalog(remote.workspace, CONNECTION_ID, 1);
    await settle();

    remote.watches[0]?.publish(true);
    const opening = remote.opens[0];
    opening?.options.onData?.(
    ...
    opening?.result.resolve(opening.channel);
    await settle();
    await vi.advanceTimersByTimeAsync(0);

    expect(opening?.send).toHaveBeenCalledWith(
      "icons Alacritty\nbrave-browser",
    );
  });
  ...
  it("keeps only one open in flight across duplicate presence updates", async () => {
    ...
    await settle();
    expect(xdgDesktopHandle(CONNECTION_ID)).not.toBeNull();
  });
  ...
  it("closes a stale pending resolution after an absent/present flap", async () => {
    ...
    await settle();
    const replacement = xdgDesktopHandle(CONNECTION_ID);
    expect(replacement).not.toBeNull();

    first?.result.resolve(first.channel);
    await settle();
    expect(first?.close).toHaveBeenCalledTimes(1);
    expect(xdgDesktopHandle(CONNECTION_ID)).toBe(replacement);
  });
  ...
  it("reopens an unexpectedly closed channel while its name stays present", async () => {
    ...
    expect(xdgDesktopHandle(CONNECTION_ID)).toBeNull();
    expect(remote.opens).toHaveLength(1);
  ...
  it("generation replacement closes the earlier pending open on resolution", async () => {
    ...
    ensureXdgDesktopCatalog(remote.workspace, CONNECTION_ID, 2);
    await settle();
