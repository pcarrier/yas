...
function recordJson(value: Readonly<Record<string, unknown>>): string {
  return json(
    Object.fromEntries(
...
export function workspaceSessionPatch(
  ...
  if (recordJson(stored.assignments) !== recordJson(current.assignments)) {
    workspace.assignments = current.assignments;
  }
  if (
    recordJson(stored.parkedPlacements ?? {}) !==
    recordJson(current.parkedPlacements ?? {})
  ) {
    workspace.parkedPlacements = current.parkedPlacements ?? {};
  }
  if ((stored.soloedPaneId ?? null) !== (current.soloedPaneId ?? null)) {
    workspace.soloedPaneId = current.soloedPaneId ?? null;
  }
