...
export function workspaceSessionPatch(
  ...
  if (stored.focusedPaneId !== current.focusedPaneId) {
    workspace.focusedPaneId = current.focusedPaneId;
  }
...
/**
 * Serialize workspace persistence for one attached session.
 *
...
export class WorkspaceSessionPatchSequencer {
  ...
  submit(
    ...
    if (this.target !== target || !this.committed) {
      // A newly attached session starts from its restored UI. Never compare it
      // with the previous session's baseline.
