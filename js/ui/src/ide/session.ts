...
function gitClosedText(reason: number): string {
  switch (reason) {
    case GIT_CLOSED_CLIENT_REQUEST:
      return t("ide.gitClosedByClient");
    case GIT_CLOSED_REPO_GONE:
      return t("ide.gitRepositoryGone");
    case GIT_CLOSED_PERMISSION_LOST:
      return t("ide.gitPermissionLost");
    case GIT_CLOSED_RESOURCE_LIMIT:
      return t("ide.gitFileWatchLimit");
    default:
      return t("ide.gitWatchFailed");
  }
}
...
function buildSession(
    ...
    workspace
      ...
      .catch((e: unknown) => {
        ...
        if (isTransientConnError(e) && fsRetries++ < MAX_OPEN_RETRIES) {
          ...
          setFsError(t("ide.filesystemCapacityRetrying"));
    ...
    workspace
      .openRepo(connectionId, desc.path, {
        ...
        onClosed: (reason: number) => {
          ...
          setGitHandle(null);
          setGitError(
            tp("ide.repositoryWatchClosed", { reason: gitClosedText(reason) }),
          );
        },
      })
      .then((h) => {
        ...
        if (page.status !== GIT_STATUS_OK) {
          setLogSpecError(
            page.status === GIT_STATUS_NOT_FOUND
              ? tp("log.cannotResolve", { spec })
              : tp("log.unavailable", {
                  detail: gitStatusText(page.status),
                }),
          );
          return;
        }
  ...
  function fsForOps(): YasNativeFsSyncHandle {
    const h = rootFs();
    if (!h) throw new Error(t("ide.fileTreeDisconnected"));
    return h;
  }
}
