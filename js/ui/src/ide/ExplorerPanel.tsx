...
function MenuItem(props: {
  ...
  return (
    <TapArea
      ...
      // The click-away listener excludes this menu, so mouse and touch can
...
export function ExplorerPanel(props: {
  ...
  // Match the terminal/editor grid rather than estimating leading from the
  // font size. Smaller change labels get their own line box within that row.
  const [metricsVersion, setMetricsVersion] = createSignal(0);
  const textMetrics = createMemo(() => {
    metricsVersion();
    return {
      row: measureCell(props.fontFamily, props.fontSize).h,
      change: measureCell(props.fontFamily, props.scale.sm).h,
    };
  });
  onMount(() => {
    const remeasure = () => setMetricsVersion((version) => version + 1);
    // A loaded face or a new display/zoom DPR can change cell metrics without
    // changing the selected font's name or size.
  ...
  const rowBase = (depth: number): JSX.CSSProperties => ({
    ...
    // Ellipsis clips to this box, which must include the full font descent.
    "line-height": `${textMetrics().row}px`,
    color: props.theme.fg,
  });
              ...
              {(c) => (
                <TapArea
                  style={{
                    ...
                    "font-family": props.fontFamily,
                    "font-size": `${props.scale.sm}px`,
                    "line-height": `${textMetrics().change}px`,
                    color: props.theme.fg,
                  }}
