...
export const scanIndString = new ExternalTokenizer((input) => {
  for (let afterDollar = false, afterApostrophe = false, i = 0; ; i++) {
    ...
    if (next < 0) {
      ...
      // Inside '' … '', a doubled apostrophe is only the terminator when
      // what follows isn't one of Nix's '' escapes:
      //   ''$   a literal "$"  (so ''${ is a literal "${", not interpolation)
      //   '''   a literal "''"
      //   ''\X  the escape X   (''\n, ''\t, ''\\ …)
      // Upstream treated every '' as the end, so any shell snippet using
      // ''${VAR} terminated the string early and desynced the rest of the
      // file. We are at the SECOND apostrophe here, so peek past it.
      const escaped = input.peek(1);
      if (
        escaped === dollar ||
        escaped === apostrophe ||
        escaped === backslack
      ) {
        // Flush preceding content first; the next call re-enters on the ''.
        if (i > 1) {
          input.acceptToken(indStrContent, -1);
          break;
        }
        // ''\X is four characters, ''$ and ''' are three. Offsets are from
        // the current position (the second apostrophe, index 1).
        input.acceptToken(indEscapeSequence, escaped === backslack ? 3 : 2);
        break;
      }
