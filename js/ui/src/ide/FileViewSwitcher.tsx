...
const TABS: { view: FileView; label: string; title: string }[] = [
  { view: "editor", label: "fileView.edit", title: "fileView.editorTitle" },
  {
    view: "preview",
    label: "fileView.preview",
    title: "fileView.previewTitle",
  },
  {
    view: "staged",
    label: "fileView.staged",
    title: "fileView.stagedTitle",
  },
  {
    view: "unstaged",
    label: "fileView.unstaged",
    title: "fileView.unstagedTitle",
  },
  {
    view: "worktree",
    label: "fileView.all",
    title: "fileView.allTitle",
  },
];
...
export function FileViewSwitcher(props: {
  ...
  return (
    ...
    >
      <For each={tabs()}>
        {(tab) => {
          const on = () => active() === tab.view;
          return (
            <TapButton
              onClick={() => {
                if (!on()) props.onOpenTile(assignmentFor(tab.view));
              }}
              title={t(tab.title)}
              style={{
                display: "inline-flex",
