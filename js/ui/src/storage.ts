...
/** Whether the right preview/sidebar dock is open ("1"/"0"). */
export const PREVIEW_PANEL_OPEN_KEY = "yas.previewPanelOpen";
...
/** Right-sidebar visibility is browser chrome, shared across workspace
 * sessions. The stored session value is only a migration fallback until the
 * user explicitly opens or closes it on this browser. */
export function preferredPreviewPanelOpen(fallback: boolean): boolean {
  const raw = readStorage(PREVIEW_PANEL_OPEN_KEY);
 ...
 *
 * Absent (first run) collapses Problems. Branches and Commit Log follow the
 * current repository instead: createDockSections discards legacy Git collapse
 * entries on restore, and persists only the global Files/Problems choices.
 */
