...
export interface WorkspaceSessionControllerOptions {
  ...
  /** Snapshot after credential removal, before canonical session replacement. */
  initialHash: string;
}
...
export function createWorkspaceSessionController(
  ...
  let pruneAttemptKey = "";
  let pruneAttempts = 0;
  let initialDefaultCandidate: StoredWorkspaceSession | null = null;
  ...
  const enqueueRemoteActiveFor = (
    ...
    // Keep the per-session queue usable after a failed durable mutation. The
  ...
  const selectSession = async (
    ...
    try {
      ...
      if (!value) throw new Error("Workspace session not found");
      ...
      if (!value) {
        ...
        throw new Error(
          quarantined
            ? "Workspace session is quarantined and cannot be opened"
      ...
      setBinding({
        ...
        current: () => {
          ...
          // new tab was selected. Never let it read or patch the new session.
          return lastValue;
        },
  ...
  const selectBase = async (
    ...
    // Only an absent device record gets an automatic Default. Two browser
    // tabs can race first boot, so each creates a uniquely named candidate and
    // the device store atomically claims exactly one of them.
    initialDefaultCandidate ??= await options.store.create({
      name: "Default",
      activeRemotes: [],
    });
    syncStore();
    const candidate = initialDefaultCandidate;
    if (disposed || reservedTicket !== operation) {
      initialDefaultCandidate = null;
    ...
    try {
      ...
      cleanupError = `Could not remove an unused Default session: ${message(
        cause,
      )}`;
    }
  ...
  const bootstrap = async () => {
    ...
    try {
      ...
      // lifecycle. Keep them active even when the initially requested session
      ...
      // session id reaches an address bar now. Otherwise the attachment comes
  ...
  const controller: WorkspaceSessionController = {
    ...
    async create(name = "New workspace") {
      setActionError(null);
      try {
