...
export function createWorkspaceSessionController(
  ...
  const bootstrap = async (showLoading: boolean) => {
    if (disposed) return;
  ...
  const startBootstrap = (showLoading: boolean) => {
    if (started) return Promise.resolve();
    startPromise ??= bootstrap(showLoading).finally(() => {
      startPromise = null;
    });
    return startPromise;
  };

  const start = () => startBootstrap(true);
  ...
  const retry = async () => {
    ...
    setActionError(null);
    if (!started) {
      // An explicit retry happens inside the workspace manager. Keep that
      // context on screen while the stores reconnect instead of replacing the
      // entire UI with the first-boot loading treatment.
      await startBootstrap(false);
      return;
    }
    await Promise.all([options.store.start(), options.deviceStore.start()]);
    syncStore();
