...
function readStoredDeviceId(storage: WorkspaceSessionDeviceIdStorage) {
  ...
  try {
    ...
    throw storageError(t("sessions.localStorageRequired"), cause);
  }
...
function persistDeviceId(
  ...
  try {
    ...
    throw storageError(t("sessions.deviceIdPersistFailed"), cause);
  }
}
...
function openDeviceIdDatabase(factory: IDBFactory): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error ?? new Error(t("sessions.deviceDbOpenFailed")));
    request.onblocked = () =>
      reject(new Error(t("sessions.deviceDbUpgradeBlocked")));
  });
}
    ...
    try {
      return await new Promise<string>((resolve, reject) => {
        ...
        transaction.onerror = () =>
          reject(
            transaction.error ??
              new Error(t("sessions.deviceIdCoordinationFailed")),
          );
        transaction.onabort = () =>
          reject(
            transaction.error ??
...
/**
 * Return this browser device's durable workspace identity.
 *
 ...
 * to localStorage before App constructs either backend workspace store.
 */
export async function getOrCreateWorkspaceSessionDeviceId(
  ...
  if (!coordinator) {
    throw new Error(t("sessions.crossTabSupportRequired"));
  }
  ...
  try {
    ...
    throw storageError(t("sessions.deviceIdAcrossTabsFailed"), cause);
  }
  if (!isWorkspaceSessionDeviceId(claimed)) {
    throw new Error(t("sessions.deviceCoordinatorInvalidId"));
  }
