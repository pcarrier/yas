...
export function SwitcherOverlay(props: {
  ...
  onRemoveLayout?: (layout: WorkspaceLayout) => void;
  ...
  /** Attached backend workspace, used for canonical share URLs. */
  ...
  const layoutChoices = createMemo(() => {
    ...
    return searching()
      ? recent.filter((layout) => layout.name.toLowerCase().includes(needle))
      : recent;
  });
  ...
  const buildSections = (): SwitcherSection[] => {
    ...
    const recent = layoutChoices().map<LayoutItem>((layout, index) => ({
      type: "layout",
      key: `layout:recent:${index}`,
      title: layout.name,
      subtitle: tp("switcher.paneCount", { count: leafCount(layout.root) }),
      layout,
    }));
    ...
    if (props.onOpenWeb) {
      actions.push({
        type: "action",
        key: "action:open-web",
        title: t("switcher.newWebPane"),
        subtitle: t("switcher.newWebPaneDesc"),
        action: "open-web",
      });
    }
    ...
    if (props.workspaceSessionId || workspaceSessionIdFromHash(location.hash)) {
      actions.push({
        ...
        title: t("switcher.shareUrl"),
        subtitle: t("switcher.shareUrlDesc"),
        action: "share-url",
      });
    }
    if (props.onManageSessions) {
      actions.push({
        ...
        title: t("switcher.sessions"),
        subtitle: t("switcher.sessionsDesc"),
    ...
    }
    {
      ...
      if (!isStandalone) {
        actions.push({
          ...
          title: t("switcher.installApp"),
          subtitle: getInstallPrompt()
            ? t("switcher.installAppDesc")
            : t("switcher.installAppBrowserDesc"),
          action: "install-app",
        });
      }
    }
    ...
    // being offered for here.
    {
      ...
      for (const remote of xdgDesktopCatalogs(connectionIds())) {
  ...
  let itemsScrollerRef!: HTMLDivElement;
  onMount(() => {
  ...
  createEffect(() => {
    ...
    if (el && itemsScrollerRef) revealSwitcherItem(itemsScrollerRef, el);
  });
          ...
          >
            <div
              ...
              style={{
                ...
                flex: "1",
                overflow: "auto",
                // Stop a wheel or touch pan at either end of the menu from
                // chaining into the document and moving the workspace tabs.
                              ...
                              >
                                ...
                                >
                                  ...
                                  <Show
                                    when={
                                      item.type === "layout" &&
                                      props.activeLayout != null &&
                                      sameLayoutTree(
                                        props.activeLayout.root,
                                        (item as LayoutItem).layout.root,
                                      )
                                    }
                                  >
                                    <mark style={ui.badge}>
