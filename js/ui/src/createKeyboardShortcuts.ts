...
export interface KeyboardShortcutHandlers {
  ...
  forwardPrefix: () => void;
  /**
   * Split the focused slot in two. Only reached with no layout on screen:
   * LayoutContainer binds the same two keys over these while a layout is live,
   * because splitting one pane of a tree is not the same operation as turning
...
export function createKeyboardShortcuts(h: KeyboardShortcutHandlers): void {
  onMount(() => {
    ...
    const bindings: [string, () => void, string][] = [
      ...
      ["c", () => newTerminal(true), t("help.newTerminal")],
      ["C", () => newTerminal(false), t("help.newTerminalHere")],
      ...
      ["d", h.toggleDebug, t("help.debugPanel")],
      ["w", () => h.toggleOverlay("web"), t("help.webPane")],
      ["q", backgroundFocused, t("help.removeFromPane")],
      ["x", closeFocused, t("help.closeTerminal")],
      ["[", () => cycleWindow(-1), t("help.previousWindow")],
      ["]", () => cycleWindow(1), t("help.nextWindow")],
      ["ArrowLeft", h.navigateBack, t("help.navBack")],
      ["ArrowRight", h.navigateForward, t("help.navForward")],
      // tmux's own axes: -h puts the new pane beside this one, -v below it.
