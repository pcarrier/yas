...
type OneShotModifierSurface = {
  ctrlModifier: boolean;
  setCtrlModifier: (active: boolean) => void;
};

/**
 * Feed a key to the prefix state machine with the mobile toolbar's one-shot
 * Ctrl applied. The toolbar state lives on the focused terminal/surface, not
 * on the DOM KeyboardEvent, so capture-phase workspace shortcuts otherwise
 * see a plain `b` and the pane later receives ^B.
 */
export function handleOneShotCtrlPrefixKey(
  event: PrefixChord,
  surface: OneShotModifierSurface | null,
): boolean {
  const oneShotCtrl =
    !!surface?.ctrlModifier &&
    !event.ctrlKey &&
    !event.metaKey &&
    !event.altKey &&
    event.key.length === 1;
  const handled = handlePrefixKey(
    oneShotCtrl
      ? {
          key: event.key,
          code: event.code,
  ...
  if (handled && oneShotCtrl) surface.setCtrlModifier(false);
  return handled;
}
...
export function createKeyboardShortcuts(h: KeyboardShortcutHandlers): void {
  onMount(() => {
    ...
    const oneShotSurface = (
      target: EventTarget | null,
    ): OneShotModifierSurface | null => {
      const element = eventElement(target);
      return terminalSurfaceForInput(element) ?? surfaceCanvasForInput(element);
    };
    ...
    const consumePrefix = (
      ...
      if (!handleOneShotCtrlPrefixKey(chord, surface)) return false;
      event.preventDefault();
      event.stopImmediatePropagation();
      return true;
    };
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      const surface = oneShotSurface(e.target);
      // WebKit can label a hardware or toolbar-generated Ctrl+B as an IME
      // keydown (229). A genuine prefix chord is still fully identified, so
      // let it arm before excluding composition traffic.
      if (
        (isPrefixChord(e) ||
          (surface?.ctrlModifier &&
            !e.ctrlKey &&
      ...
      if (consumePrefix(e, e, surface)) return;
    ...
    const inputHandler = (e: InputEvent) => {
      if (
        ...
        return;
      const characters = Array.from(e.data);
      if (characters.length !== 1) return;
      const surface = oneShotSurface(e.target);
      if (!surface?.ctrlModifier && !prefixArmed()) return;
      consumePrefix(
        e,
        {
          key: characters[0],
          ...
          metaKey: false,
          altKey: false,
          shiftKey: characters[0] !== characters[0].toLocaleLowerCase(),
        },
        surface,
      );
    };
    ...
    window.addEventListener("beforeinput", inputHandler, true);
    onCleanup(() => {
      window.removeEventListener("keydown", handler, true);
      window.removeEventListener("beforeinput", inputHandler, true);
    });
  });
}
