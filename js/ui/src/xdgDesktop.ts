/**
 * Client mirror for the `yas.xdg-desktop.v1` channel served by the XDG desktop
 * supervisor extension (`extensions/xdg-desktop`).
 *
 ...
 *
 * Icons are the exception to "complete state": they are asked for in bounded
 * batches and answered one message per id. The page-lifetime catalogue asks
 * for the complete shelf immediately, then panels reuse the retained artwork.
 *
...
export const XDG_DESKTOP_CHANNEL = "yas.xdg-desktop.v1";

/** What the supervisor is doing about one application. */
export type XdgDesktopPhase = "running" | "backoff" | "starting" | "stopped";

/** One application the session manages. */
export interface XdgDesktopApp {
  /** Desktop-entry id — the name `@xdg-desktop enable <id>` takes. */
  readonly id: string;
  readonly name: string;
  readonly enabled: boolean;
  readonly phase: XdgDesktopPhase;
...
export interface XdgDesktopCatalogEntry {
  readonly id: string;
  readonly name: string;
}
 ...
 */
export const XDG_DESKTOP_MAX_ARTWORK_FILE_BYTES = 1024 * 1024;
 ...
 * bounded page.
 */
export const XDG_DESKTOP_ARTWORK_READ_CREDIT = 2 * 1024 * 1024;
...
export const XDG_DESKTOP_MAX_APPS = 1024;
export const XDG_DESKTOP_MAX_CATALOG_ENTRIES = 4096;
export const XDG_DESKTOP_MAX_STATE_BYTES = 2 * 1024 * 1024;
export const XDG_DESKTOP_MAX_ID_CHARS = 255;
export const XDG_DESKTOP_MAX_NAME_CHARS = 1024;
export const XDG_DESKTOP_MAX_SOCKET_CHARS = 255;
export const XDG_DESKTOP_MAX_ICON_PATH_CHARS = 4096;

/** Keep the launcher's resolved shelf, bounded primarily by actual blob bytes. */
export const XDG_DESKTOP_ARTWORK_MAX_ENTRIES =
  XDG_DESKTOP_MAX_CATALOG_ENTRIES - 64;
export const XDG_DESKTOP_ARTWORK_MAX_BYTES = 32 * 1024 * 1024;
export const XDG_DESKTOP_ARTWORK_LOAD_MAX_ROUNDS = Math.ceil(
  XDG_DESKTOP_MAX_CATALOG_ENTRIES / ICON_BATCH,
);

/** Outstanding requests are peer-derived IDs and therefore bounded too. */
export const XDG_DESKTOP_ICON_REQUEST_MAX_ENTRIES =
  XDG_DESKTOP_MAX_CATALOG_ENTRIES;
export const XDG_DESKTOP_ICON_REQUEST_MAX_BYTES = 1024 * 1024;
export const XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES =
  XDG_DESKTOP_MAX_CATALOG_ENTRIES;
export const XDG_DESKTOP_ICON_QUEUE_MAX_BYTES = 1024 * 1024;
export const XDG_DESKTOP_COMMAND_QUEUE_MAX_ENTRIES = 64;
export const XDG_DESKTOP_COMMAND_QUEUE_MAX_BYTES = 64 * 1024;

export interface XdgDesktopOptions {
  onClosed?(reason: number, detail: string): void;
}

export interface XdgDesktopHandle extends ReactiveStore {
...
function phaseOf(value: unknown): XdgDesktopPhase {
...
function validXdgDesktopId(value: unknown): value is string {
  return (
    typeof value === "string" &&
    value.length > 0 &&
    value.length <= XDG_DESKTOP_MAX_ID_CHARS &&
    !value.includes("\n") &&
    !value.includes("\r") &&
    !value.includes("\0")
  );
...
function appOf(value: unknown): XdgDesktopApp | null {
  if (typeof value !== "object" || value === null) return null;
  const record = value as Record<string, unknown>;
  if (!validXdgDesktopId(record.id)) return null;
  return {
    id: record.id,
    name: boundedText(record.name, record.id, XDG_DESKTOP_MAX_NAME_CHARS),
    ...
    socket:
      typeof record.socket === "string"
        ? record.socket.slice(0, XDG_DESKTOP_MAX_SOCKET_CHARS)
        : undefined,
  };
}

function entryOf(value: unknown): XdgDesktopCatalogEntry | null {
  if (typeof value !== "object" || value === null) return null;
  const record = value as Record<string, unknown>;
  if (!validXdgDesktopId(record.id)) return null;
  return {
 ...
 *
 * Separate from {@link openXdgDesktop} so it can be driven from a recorded
 * transcript and tested without a connection.
 */
export class XdgDesktopMirror implements ReactiveStore {
  readonly #notifier = new Notifier();
  #apps: XdgDesktopApp[] = [];
  #appsBytes = 0;
  #catalog: XdgDesktopCatalogEntry[] = [];
  #catalogBytes = 0;
  #artwork = new Map<
    string,
    {
  ...
  get apps(): readonly XdgDesktopApp[] {
    return this.#apps;
  }

  get catalog(): readonly XdgDesktopCatalogEntry[] {
    return this.#catalog;
  }
  ...
  setIcon(id: string, url: string | null, blobBytes = 0): boolean {
    if (
      !validXdgDesktopId(id) ||
      !this.acceptsId(id) ||
      !Number.isFinite(blobBytes) ||
      blobBytes < 0 ||
      blobBytes > XDG_DESKTOP_MAX_ARTWORK_FILE_BYTES
    ) {
  ...
  acceptsId(id: string): boolean {
    if (!validXdgDesktopId(id)) return false;
  ...
  #storeArtwork(
    ...
    this.#artworkBytes += this.#artworkCost(id, artwork);
    while (
      this.#artwork.size > XDG_DESKTOP_ARTWORK_MAX_ENTRIES ||
      this.#artworkBytes > XDG_DESKTOP_ARTWORK_MAX_BYTES
    ) {
  ...
  apply(payload: Uint8Array): void {
    ...
    if (record.type === "icon") {
      if (!validXdgDesktopId(record.id) || !this.acceptsId(record.id)) return;
      const path = record.path;
      if (
        typeof path === "string" &&
        path.length > 0 &&
        path.length <= XDG_DESKTOP_MAX_ICON_PATH_CHARS &&
        !path.includes("\0")
      ) {
    ...
    if (hasApps) {
      const budget = Math.max(
        0,
        XDG_DESKTOP_MAX_STATE_BYTES - (hasCatalog ? 0 : this.#catalogBytes),
      );
    ...
    if (hasCatalog) {
      const parsed = boundedRows(
        ...
        catalogEntryBytes,
        XDG_DESKTOP_MAX_CATALOG_ENTRIES,
        Math.max(0, XDG_DESKTOP_MAX_STATE_BYTES - this.#appsBytes),
      );
...
export async function openXdgDesktop(
  connection: ChannelOpener,
  options: XdgDesktopOptions = {},
): Promise<XdgDesktopHandle> {
  ...
  const mirror = new XdgDesktopMirror({
  ...
  const channelHandle = await connection.connectChannel(XDG_DESKTOP_CHANNEL, {
  ...
  const command = (line: string): void => {
    ...
    if (
      pendingCommands.length >= XDG_DESKTOP_COMMAND_QUEUE_MAX_ENTRIES ||
      bytes > XDG_DESKTOP_COMMAND_QUEUE_MAX_BYTES - pendingCommandBytes
    ) {
      return;
    }
  ...
  const markAsked = (id: string, now: number): void => {
    ...
    askedBytes += keyedBytes(id);
    while (
      asked.size > XDG_DESKTOP_ICON_REQUEST_MAX_ENTRIES ||
      askedBytes > XDG_DESKTOP_ICON_REQUEST_MAX_BYTES
    ) {
  ...
  const worthAsking = (id: string, now: number): boolean => {
    if (!validXdgDesktopId(id) || !mirror.acceptsId(id)) return false;
  ...
  loadArtwork = async () => {
    ...
    try {
      ...
      for (
        let guard = 0;
        guard < XDG_DESKTOP_ARTWORK_LOAD_MAX_ROUNDS;
        guard += 1
      ) {
        if (closed) return;
        ...
        const records = await connection.readFiles(
          [wanted.map((entry) => entry.path)],
          {
            ...
            maxBytes: XDG_DESKTOP_ARTWORK_READ_CREDIT,
          },
        );
        if (closed) return;
        for (const [index, record] of records.entries()) {
          ...
          if (
            ...
            record.content.byteLength > XDG_DESKTOP_MAX_ARTWORK_FILE_BYTES
          ) {
          ...
          // Start decode while the launcher is closed. Object URLs keep the
          // bytes warm; decode() keeps first paint from paying image parsing.
  ...
  return {
    ...
    requestIcons: (ids: readonly string[]) => {
      ...
      for (const id of ids) {
        ...
        if (
          queued.size >= XDG_DESKTOP_ICON_QUEUE_MAX_ENTRIES ||
          bytes > XDG_DESKTOP_ICON_QUEUE_MAX_BYTES - queuedBytes
        ) {
          continue;
        }
        queued.add(id);
    ...
    enable: (id: string) => {
      if (validXdgDesktopId(id)) command(`enable ${id}`);
    },
    disable: (id: string) => {
      if (validXdgDesktopId(id)) command(`disable ${id}`);
    },
    start: (id: string) => {
      if (validXdgDesktopId(id)) command(`start ${id}`);
    },
    stop: (id: string) => {
      if (validXdgDesktopId(id)) command(`stop ${id}`);
    },
    forget: (id: string) => {
      if (validXdgDesktopId(id)) command(`forget ${id}`);
    },
