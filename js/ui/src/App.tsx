...
interface EdgeWebTransportConfig {
  url: string;
...
async function discoverEdgeWebTransport(): Promise<EdgeWebTransportConfig | null> {
  if (typeof WebTransport === "undefined") return null;
  try {
    const response = await fetch("/edge-transport.json", { cache: "no-store" });
...
function ConnectedApp(props: {
  ...
  const edgeTransport = props.edgeWebTransport
    ? new YasWebTransportTransport(
        ...
        props.passphrase,
        props.edgeWebTransport.certificateHash
          ? { serverCertificateHash: props.edgeWebTransport.certificateHash }
          : {},
      )
