/** Durable per-browser-device workspace-session attachment ordering. */
...
export interface StoredWorkspaceSessionDevice {
  version: 1;
  deviceId: string;
  /** Ordered, unique durable workspace-session IDs attached on this device. */
...
export function parseWorkspaceSessionDeviceId(value: unknown): string {
  if (!isWorkspaceSessionDeviceId(value))
    throw new WorkspaceSessionValidationError(
      "workspace session device id is not a canonical lowercase UUID",
    );
  return value;
}
...
export function parseStoredWorkspaceSessionDevice(
  ...
  const input = strictObject(
    ...
    "workspace session device",
  );
  if (input.version !== WORKSPACE_SESSION_DEVICE_VERSION)
    invalid("workspace session device version is unsupported");
...
function sessionIds(value: unknown): string[] {
  ...
  if (value.length > WORKSPACE_SESSION_DEVICE_MAX_ATTACHED_SESSIONS)
    invalid("attached workspace session count exceeds its limit");
  const result = value.map((id) => {
    if (!isWorkspaceSessionId(id))
      invalid("attached workspace session id is not a canonical UUID");
    return id;
  });
 ...
 * session selection remains URL/tab-local; this record only owns membership
...
export class WorkspaceSessionDeviceStore {
  ...
  constructor(
    ...
    if (!Number.isInteger(retries) || retries < 0 || retries > 10)
      invalid("workspace session device CAS retry count is invalid");
  ...
  start(): Promise<void> {
    if (this.snapshot.status === "closed")
      return Promise.reject(
        new Error("Workspace session device store is closed"),
      );
  ...
  close(): void {
    ...
    this.ownedKv = null;
    const error = new Error("Workspace session device store is closed");
  ...
  async attach(
    ...
    if (!result) throw new Error("Workspace session device attach failed");
    return result;
  }

  /**
   * Atomically initialize an absent device record with one session. Existing
  ...
  async claimInitialSession(
    ...
    return this.enqueueStateOperation(async () => {
      ...
      throw (
        lastConflict ?? new Error("Workspace session device claim conflicted")
      );
    });
  }
  ...
  /**
   * Remove IDs absent from a complete authoritative session catalogue.
  ...
  private nextRecord(
    ...
    if (!Number.isSafeInteger(updatedAtUnixMs))
      invalid("workspace session device timestamp overflowed");
    return parseStoredWorkspaceSessionDevice({
      ...current,
      attachedSessionIds,
      updatedAtUnixMs,
  ...
  private async mutate(
    ...
    return this.enqueueStateOperation(async () => {
      ...
      throw (
        lastConflict ?? new Error("Workspace session device update conflicted")
      );
    });
  }
  ...
  private async operationalKv(): Promise<WorkspaceSessionKv> {
    ...
    if (!this.kv) throw new Error("Workspace session device KV is unavailable");
    return this.kv;
  }
  ...
  private waitUntilReady(): Promise<void> {
    ...
    if (this.snapshot.status === "error" || this.snapshot.status === "closed")
      return Promise.reject(
        this.snapshot.error ??
          new Error("Workspace session device store is unavailable"),
      );
  ...
  private openWatch(): void {
    ...
    try {
      ...
      if (!this.kv)
        throw new Error("Workspace session device KV is unavailable");
      kv = this.kv;
    } catch (error) {
    ...
    this.setStatus(
      ...
      invalidRecords.length === 0
        ...
        : new WorkspaceSessionValidationError(
            `${invalidRecords.length} workspace session device record(s) quarantined`,
          ),
    );
    this.resolveStart?.();
    this.resolveStart = null;
  ...
  private async loadEntry(
    ...
    if (entry.size > WORKSPACE_SESSION_DEVICE_MAX_DOCUMENT_BYTES)
      invalid("workspace session device document exceeds its byte limit");
...
function validateValidIds(
  ...
  if (ids.length > WORKSPACE_SESSION_MAX_CATALOG_ENTRIES)
    invalid("valid workspace session catalogue exceeds its limit");
  for (const id of ids) assertSessionId(id);
  if (new Set(ids).size !== ids.length)
    invalid("valid workspace session catalogue contains a duplicate");
  return new Set(ids);
}
...
function decodeDocument(
  ...
  try {
    ...
    throw new WorkspaceSessionValidationError(
      `workspace session device JSON is invalid: ${asError(error).message}`,
    );
  }
  const record = parseStoredWorkspaceSessionDevice(value);
  if (record.deviceId !== expectedDeviceId)
    invalid("workspace session device key and document id do not match");
  return record;
}
...
function indexed(
  ...
  if (
    ...
    invalid("workspace session device byte length is invalid");
  return {
    record,
