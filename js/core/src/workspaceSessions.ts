/** Durable, user-owned workspace sessions stored in the home server KV. */
...
export class WorkspaceSessionNotFoundError extends Error {
  constructor(readonly id: string) {
    super(`Workspace session ${id} was not found`);
...
/** Conservative defaults for callers creating an empty session. */
...
export function parseStoredWorkspaceSession(
  ...
  if (record.version !== WORKSPACE_SESSION_VERSION)
    invalid("workspace session version is unsupported");
  const id = boundedString(record.id, 36, "workspace session id");
  assertSessionId(id);
  ...
  if (name !== name.trim()) invalid("workspace session name is not trimmed");
...
function assertSessionId(id: string): void {
  if (!isWorkspaceSessionId(id))
    invalid("workspace session id is not a canonical lowercase UUID");
}
...
export class WorkspaceSessionStore {
  ...
  constructor(
    ...
    if (!Number.isInteger(retries) || retries < 0 || retries > 10)
      invalid("workspace session CAS retry count is invalid");
  ...
  start(): Promise<void> {
    if (this.snapshot.status === "closed")
      return Promise.reject(new Error("Workspace session store is closed"));
  ...
  close(): void {
    ...
    this.ownedKv = null;
    const error = new Error("Workspace session store is closed");
    ...
    return this.enqueueStateOperation(async () => {
      ...
      for (let attempt = 0; attempt <= this.casRetries; attempt++) {
        ...
        if (
          ...
          throw new Error("Workspace session catalogue limit exceeded");
      ...
      throw new Error("Could not allocate a unique workspace session id");
    });
  }
  ...
  async setRemoteActive(
    ...
    if (typeof active !== "boolean")
      throw new WorkspaceSessionValidationError(
        "workspace session remote membership must be boolean",
      );
  ...
  private async mutateSession(
    ...
    return this.enqueueStateOperation(async () => {
      ...
      throw lastConflict ?? new Error("Workspace session update conflicted");
    });
  }
  ...
  async delete(id: string): Promise<void> {
    ...
    return this.enqueueStateOperation(async () => {
      ...
      throw lastConflict ?? new Error("Workspace session delete conflicted");
    });
  }
  ...
  private async operationalKv(): Promise<WorkspaceSessionKv> {
    ...
    if (!this.kv) throw new Error("Workspace session KV is unavailable");
    return this.kv;
  }
  ...
  private waitUntilReady(): Promise<void> {
    ...
    if (this.snapshot.status === "error" || this.snapshot.status === "closed")
      return Promise.reject(
        this.snapshot.error ?? new Error("Workspace session store is closed"),
      );
    return new Promise((resolve, reject) => {
      const unsubscribe = this.subscribe(() => {
        if (this.snapshot.status === "ready") {
          ...
          reject(
            this.snapshot.error ??
              new Error("Workspace session store is closed"),
          );
        }
      });
    });
    ...
    for (let index = 0; index < records.length; index++) {
      ...
      if (index >= WORKSPACE_SESSION_MAX_CATALOG_ENTRIES) {
        quarantine(
          quarantined,
          key,
          "workspace session catalogue limit exceeded",
        );
        continue;
      }
      const id = key.slice(WORKSPACE_SESSION_KEY_PREFIX.length);
      if (!isWorkspaceSessionId(id)) {
        quarantine(
          quarantined,
          key,
          "workspace session key suffix is not a UUID",
        );
        continue;
      }
        ...
        if (workspaceSessionHashesEqual(entry.hash, pending.indexed.hash)) {
          ...
          if (
            ...
            quarantine(
              quarantined,
              key,
              "workspace session retained-byte limit exceeded",
            );
            continue;
          }
          retainedBytes += pending.indexed.retainedBytes;
      ...
      this.setStatus(
        ...
        quarantined.size === 0
          ...
          : new WorkspaceSessionValidationError(
              `${quarantined.size} workspace session record(s) quarantined`,
            ),
      );
      this.resolveInitial?.();
      this.resolveInitial = null;
  ...
  private async loadMirrorEntry(
    ...
    if (entry.size > WORKSPACE_SESSION_MAX_DOCUMENT_BYTES)
      invalid("workspace session document exceeds its byte limit");
  ...
  private assertInstallCapacity(id: string, byteLength: number): void {
    if (!this.entries.has(id)) {
      ...
      if (
        ...
        invalid("workspace session catalogue limit exceeded");
    }
  ...
  private assertLocalCapacity(id: string, byteLength: number): void {
    ...
    if (
      ...
      invalid("workspace session retained-byte limit exceeded");
  }
  ...
  private updateReadyQuarantineError(): void {
    ...
    this.setStatus(
      ...
      this.invalidRecords.size === 0
        ? null
        : new WorkspaceSessionValidationError(
            `${this.invalidRecords.size} workspace session record(s) quarantined`,
          ),
    );
  }
...
export class WorkspaceSessionAttachment {
  ...
  private assertAttached(): void {
    if (!this.attached) throw new Error("Workspace session is detached");
  }
...
function applyPatch(
  ...
  if (!Number.isSafeInteger(updatedAtUnixMs))
    invalid("workspace session updated timestamp overflowed");
...
function retainedEstimate(byteLength: number): number {
  if (!Number.isSafeInteger(byteLength) || byteLength < 0)
    invalid("workspace session byte length is invalid");
...
function decodeDocument(
  ...
  try {
    ...
    throw new WorkspaceSessionValidationError(
      `workspace session JSON is invalid: ${asError(error).message}`,
    );
  }
  const record = parseStoredWorkspaceSession(parsed);
  if (record.id !== expectedId)
    invalid("workspace session key and document id do not match");
  return record;
}
...
function defaultRandomUUID(): string {
  ...
  if (!randomUUID)
    throw new Error("crypto.randomUUID is unavailable for session creation");
