...
describe("YAS Surface unreliable frame assembly", () => {
  ...
  it("drops a reliable frame overtaken by a newer datagram", () => {
    const view = surfaceView(4, 2n);
    ...
    view.accept(surfaceFrame(1n, 0, 2, 0x10));
    view.accept(surfaceFrame(2n, 0, 1, 0x20), true);
    expect(() => view.accept(surfaceFrame(1n, 1, 2, 0x11))).not.toThrow();
    view.accept(surfaceFrame(3n, 0, 2, 0x30));
    view.accept(surfaceFrame(3n, 1, 2, 0x31));

    expect(received).toEqual([2n, 3n]);
  });
