...
describe("audio worklet learned floor", () => {
  ...
  const playCleanly = (seconds: number): void => {
    ...
    // AudioWorklet reuses its output buffers. Mirror that here: this loop can
    // simulate minutes of playback, and allocating three typed arrays for
    // every render quantum makes the test sensitive to runner/GC speed.
