...
describe("YasSurfaceCanvas layout", () => {
  ...
  it("does not prefilter an oversized live frame to the pane", () => {
    const { surface, canvas } = attachCanvas({ resizable: true });
    const source = document.createElement("canvas");
    source.width = 1600;
    source.height = 1200;
    ...
    // remains a 1600×1200 backing canvas / 800×600 CSS box and is clipped;
    // the live path must not halve it into the pane first.
  ...
  it("clips an oversized mismatched frame without fitting or centering", () => {
    const { surface, canvas } = attachCanvas();
    setSurfaceInfo(surface, { width: 2000, height: 480, lw: 640, lh: 480 });
    canvas.width = 2000;
    canvas.height = 480;
    surface.setDisplaySize(1280, 960, 240);
    // At 2x this is a 1000×240 CSS-pixel box backed by 2000×480
    // device pixels. The 640×480 CSS-pixel pane clips its right edge.
  ...
  it.each([
    ...
    ({ scale, cssScale, cssWidth, cssHeight }) => {
      ...
      // Neither a new catalogue record nor a local pane resize may reinterpret
      // this old frame while its replacement is in flight.
  ...
  it("keeps the old frame at 1:1 while an unzoomed pane shrinks", () => {
    ...
    // The reduced container clips this old 1200x900 logical frame. It must
    // not squeeze the whole desktop into 400x300 while RESIZE is in flight.
    ...
    // Only an actual replacement frame changes the presentation extent.
    expect(canvas.style.width).toBe("1200px");
    canvas.width = 400;
    canvas.height = 300;
    (surface as unknown as { applyLayout(): void }).applyLayout();
  ...
  it("keeps the old CSS footprint while a zoomed HiDPI pane shrinks", () => {
    const { surface, canvas, acknowledgements, update } = attachResizableCanvas(
    ...
    // The old 640×480 CSS picture is clipped by the new 320×240 CSS pane.
    // Surface zoom (300/120) must not turn either footprint into 512×384.
    expect(canvas.style.width).toBe("640px");
    expect(canvas.style.height).toBe("480px");
...
describe("YasSurfaceCanvas scroll", () => {
  ...
  it("keeps a host cursor visible while a hidden guest cursor is moving", () => {
    const { surface, canvas } = attachScrolling({ appId: "brave-browser" });
    const internal = surface as unknown as {
  ...
  it("preserves an intentionally hidden cursor outside guest Brave", () => {
    const { surface, canvas } = attachScrolling({ appId: "game" });
    const internal = surface as unknown as {
      remoteCursor: { kind: "hidden" } | { kind: "named"; name: string };
    };
    internal.remoteCursor = { kind: "hidden" };
...
describe("YasSurfaceCanvas soft-keyboard input", () => {
  ...
  it("restores the IME caret on refocus without another app update", () => {
    const { surface, canvas, ta, requestTextInput } = attachTyping();
    canvas.getBoundingClientRect = () =>
    ...
    requestTextInput({
      ...
      hint: 0,
      purpose: 0,
      cursorRect: { x: 200, y: 100, width: 1, height: 20 },
    });
    expect([ta.style.left, ta.style.top, ta.style.height]).toEqual([
      "300px",
      "150px",
    ...
    // Leaving for a browser control parks the target but need not change
    // either the remote app's caret or the canvas layout.
    ta.blur();
    expect([ta.style.left, ta.style.top]).toEqual(["0px", "0px"]);
    ta.focus();
    expect([ta.style.left, ta.style.top, ta.style.height]).toEqual([
