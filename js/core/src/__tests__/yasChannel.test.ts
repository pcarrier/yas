...
function everyTruncation<T>(
  ...
  // Avoid constructing an assertion (and its stack) for all 65,536 metadata
  // bytes. Still exercise every truncated payload and report its exact offset.
  for (let end = 0; end < bytes.length; end++) {
    ...
    throw new Error(`decoder accepted truncation at ${end}/${bytes.length}`);
  }
  return decode(bytes);
}
