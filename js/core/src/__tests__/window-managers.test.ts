...
describe("the DSL carries the window manager", () => {
  ...
  it("reads a mixed tiled and floating workspace as the tiling manager", () => {
    const { root } = parseDSL(
      "scene(line(_, _), _ [18,12,54,62], _ [40,30,45,50])",
    );
    expect(windowManagerOf(root)).toBe("tiling");
    const split = root as LayoutSplit;
    expect(split.direction).toBe("workspace");
    expect(split.children[0].rect).toBeUndefined();
    expect(split.children[1].rect?.width).toBe(54);
  });
  ...
  it("rejects an ambiguous mixed scene with two tiled bases", () => {
    expect(() => parseDSL("scene(_, _, _ [10,10,40,40])")).toThrow(
      "more than one tiled base",
    );
  });
});
