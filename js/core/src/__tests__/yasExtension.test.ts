...
import {
  YAS_EXTENSION_DEPLOY_PRESERVE_ARGV_TAG,
...
describe("YAS Extension v1", () => {
  ...
  it("round-trips argument-preserving replacement DEPLOYs", () => {
    const deploy = decodeExtensionDeploy(bytes("extension.deploy.payload"));
    const replacement = {
      ...deploy,
      expectedExtensionHandle: 1n,
      ...
      extensions: [
        {
          tag: YAS_EXTENSION_DEPLOY_PRESERVE_ARGV_TAG,
          required: true,
          value: new Uint8Array(),
        },
      ],
    };
    expect(decodeExtensionDeploy(encodeExtensionDeploy(replacement))).toEqual(
      replacement,
    );
    expect(() =>
      encodeExtensionDeploy({ ...replacement, argv: [new Uint8Array([1])] }),
    ).toThrow(/preserve-argv/);
  });
