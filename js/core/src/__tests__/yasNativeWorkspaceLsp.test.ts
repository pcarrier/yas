...
describe("YasNativeWorkspaceLsp buffer admission", () => {
  ...
  it("keeps the committed projection across a catalogue reset", async () => {
    let deliver!: (snapshot: YasLspSnapshot) => void;
    const native = nativeWorkspace({
      catalog: {
        subscribe: vi.fn((listener: (snapshot: YasLspSnapshot) => void) => {
    ...
    deliver({ revision: 7n, backends: [], diagnostics: [], buffers: [] });
    deliver({ revision: 0n, backends: [], diagnostics: [], buffers: [] });

    expect(listener).toHaveBeenCalledOnce();
    expect(handle.state.revision).toBe(7n);
    expect(handle.diags.revision).toBe(7n);
  });
});
