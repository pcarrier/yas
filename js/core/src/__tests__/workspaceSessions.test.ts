...
/** A tree big enough to push a record past the inline limit. */
function largeTree(): LayoutNode {
  return {
    type: "split",
    direction: "tabs",
    children: [
      { node: { type: "leaf" }, weight: 1, label: "x".repeat(39_980) },
...
describe("workspace DTO", () => {
  it("accepts legacy solo state and validates an optional soloed pane", () => {
    const record = createDefaultStoredWorkspaceSession({ id: id(1) });
    expect(record.workspace.soloedPaneId).toBeUndefined();
    for (const soloedPaneId of [null, "1.0"]) {
      expect(
        parseStoredWorkspaceSession({
          ...record,
          workspace: { ...record.workspace, soloedPaneId },
        }).workspace.soloedPaneId,
      ).toBe(soloedPaneId);
    }
    for (const soloedPaneId of [42, "", "1".repeat(65536)]) {
      expect(() =>
        parseStoredWorkspaceSession({
          ...record,
          workspace: { ...record.workspace, soloedPaneId },
  ...
  it("copies parked frames and their reference viewports", () => {
    const placements = {
      "surface:dev:7": {
        mode: "floating",
        rect: { x: 17, y: 23, width: 43, height: 51 },
        viewport: { left: 10, top: 20, width: 1000, height: 800 },
      },
    };
    const parsed = parseWorkspacePanePlacements(placements);
    placements["surface:dev:7"].viewport.width = 760;
    placements["surface:dev:7"].rect.width = 10;
    expect(parsed["surface:dev:7"]).toEqual({
      mode: "floating",
      rect: { x: 17, y: 23, width: 43, height: 51 },
      viewport: { left: 10, top: 20, width: 1000, height: 800 },
    });
  ...
  it.each([
    ...
    { left: NaN, top: 0, width: 1000, height: 800 },
    { left: 0, top: 0, width: 1000 },
  ])("rejects invalid parked viewports: %j", (viewport) => {
    expect(() =>
      parseWorkspacePanePlacements({
        "surface:dev:7": {
          mode: "floating",
  ...
  it.each([
    null,
    [],
    { "surface:dev:7": { mode: "floating" } },
    { "surface:dev:7": { mode: "tiled", rect: {} } },
    { "surface:dev:7": { mode: "other" } },
    {
      "surface:dev:7": {
        mode: "floating",
    ...
    },
    {
      "surface:dev:7": {
        mode: "floating",
        rect: { x: 0, y: 0, width: 50, height: Infinity },
      },
    },
    {
      "surface:dev:7": {
        mode: "floating",
        rect: { x: 201, y: 0, width: 50, height: 50 },
      },
    },
  ])("rejects invalid parked placement metadata: %j", (value) => {
    expect(() => parseWorkspacePanePlacements(value)).toThrow(
      WorkspaceSessionValidationError,
    );
  });
  ...
  it("rejects structurally hostile layout trees before accepting a record", () => {
    const record = createDefaultStoredWorkspaceSession({
      id: id(12),
      nowUnixMs: 1,
    });
    const wide: LayoutNode = {
      ...
      children: Array.from({ length: LAYOUT_MAX_PANES + 1 }, () => ({
        node: { type: "leaf" },
        weight: 1,
      })),
    };
    ...
    for (const root of [wide, deep]) {
      expect(() =>
        parseStoredWorkspaceSession({
          ...record,
          workspace: {
            ...record.workspace,
            layout: { name: "hostile", root },
          },
        }),
      ).toThrow(WorkspaceSessionValidationError);
    }
...
describe("WorkspaceSessionStore", () => {
  ...
  it("merges a concurrent rename with a semantic layout/panel/remote patch", async () => {
    ...
    expect(final.workspace.soloedPaneId).toBe("1");
    expect(final.workspace.panels.leftOpen).toBe(true);
    expect(final.workspace.parkedPlacements).toEqual({
      "surface:dev:7": {
        mode: "floating",
        rect: { x: 17, y: 23, width: 43, height: 51 },
      },
    ...
    const reopened = await first.attach(created.id);
    expect(reopened.getSnapshot()?.workspace.soloedPaneId).toBe("1");
    await second.update(created.id, {
      workspace: { parkedPlacements: {}, soloedPaneId: null },
    });
    expect(
      backendRecord(backend, created.id)!.workspace.soloedPaneId,
    ).toBeNull();
    reopened.detach();
    expect(
      backendRecord(backend, created.id)!.workspace.parkedPlacements,
    ).toEqual({});
  });
  ...
  it("fetches non-inline records and quarantines bad peers without losing valid ones", async () => {
    ...
    const large = createDefaultStoredWorkspaceSession({
      ...
      name: "Large",
      nowUnixMs: 40,
      workspace: workspace({ name: "large", root: largeTree() }),
    });
    backend.setRaw(
