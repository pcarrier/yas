...
export class YasSurfaceCanvas {
   ...
   * view extent carried alongside it. A stale frame keeps its prior extent
   * while a new window or resize is settling.
   */
  ...
  setDisplaySize(
    ...
    // decoded frame size (set in presentFromStore), so a stale frame remains
    // 1:1 while the replacement is in flight.
    this.applyLayout();
  }
   ...
   *
   * A live frame is anchored at the top-left. Ordinary and stale frames retain
   * their logical extent at this viewer's scale, including adaptive streams.
   * The container clips any excess on the right and bottom.
   *
  ...
  private layoutCanvasBox(): void {
    ...
    // Present this frame's logical extent at this canvas's own scale, never
    ...
    const w = presentation?.logicalWidth
      ? presentation.logicalWidth * surfaceScale
      : (presentation?.width ?? fw);
    const h = presentation?.logicalHeight
      ? presentation.logicalHeight * surfaceScale
      : (presentation?.height ?? fh);
    // Codec-grid rounding can slightly change the encoded aspect. Map the
    // complete frame onto its logical rectangle on both axes, including input.
  ...
  private attachEvents(): void {
    ...
    this.boundDocumentVisibilityChange = () => {
      if (document.visibilityState === "hidden") this.sendPointerLeave();
    };
  ...
  private handleMouse(e: MouseEvent, type: number): void {
    ...
    if (type === SURFACE_POINTER_MOVE) this.wakeHiddenHostCursor();
  ...
  /**
   * Keep a usable local pointer while motion is waking a hidden guest cursor.
   *
   ...
   * untouched. The next real cursor event remains authoritative through the
   * cursor listener above. Until one arrives, do not reapply a hidden snapshot
   * which can survive a guest page reload indefinitely.
   */
  private wakeHiddenHostCursor(): void {
    ...
    if (
      ...
      this.surface?.appId !== "brave-browser"
    )
      return;
  ...
  private handleBlur(e: FocusEvent): void {
    ...
    // Parking changed the target without changing the app's caret or layout.
    // Refocus must restore it even if neither has changed in the meantime.
    this._imeSyncedEpoch = -1;
  ...
  private handleFocus(e: FocusEvent): void {
    ...
    if (this.remoteFocusReadyForDomFocus) return;
    const conn = this.getConn();
    if (!conn || !this.surface || !this._displaySize) return;
    this.sendRemoteFocus(conn);
  }
