/** Native KV contract used only by durable browser workspace state. */
...
export class WorkspaceSessionKvConflictError extends Error {
  ...
  constructor(hash: WorkspaceSessionHash) {
    super("workspace KV mutation conflicted");
...
export function copyWorkspaceSessionHash(
  ...
  if (hash.length !== 32) throw new Error("workspace KV hash is not 32 bytes");
