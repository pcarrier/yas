 ...
 *
 * - `tiling`: a sway-like container tree. Horizontal and vertical children
 *   divide space; tabbed and stacking containers select one child; splits nest
 *   arbitrarily.
 ...
 *   top.
 *
 * A `workspace` root remains owned by its non-floating base manager while
 * adding Sway-style floating children above it.
 *
...
export function windowManagerOf(node: LayoutNode): WindowManager {
  ...
  if (node.direction === "workspace") {
    const base = node.children.find((child) => child.rect == null)?.node;
    return base ? windowManagerOf(base) : "tiling";
  }
