 ...
 *   layout  = node
 *   node    = split | leaf
 *   split   = ("line" | "col" | "tabs" | "scroll" | "float") "(" entries ")"
 *   entries = entry ("," entry)*
 ...
 *
 * The root keyword is also the window manager. `line`/`col`/`tabs` tile: every
 * child is on screen and the weights divide the space. `scroll` is a strip
...
export interface LayoutChild {
  ...
  weight: number;
  label?: string;
  /** Only read under a `floating` parent; carried verbatim otherwise so a
   *  round trip through another manager and back does not lose the frame. */
  rect?: LayoutRect;
}
