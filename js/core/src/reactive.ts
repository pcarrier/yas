...
export class Notifier implements ReactiveStore {
  ...
  /** Advance the revision before lifecycle callbacks derived from the new
   *  value run. Producers that need that ordering pair this with
   *  {@link notify}; ordinary producers should call {@link emit}. */
  advance(): void {
    this.#revision++;
  }

  /** Notify every current listener without advancing the revision. Snapshot
   *  the set first so a listener that unsubscribes mid-notify is well-defined. */
  notify(): void {
    for (const listener of [...this.#listeners]) listener();
  }

  /** Advance the revision and notify every current listener. */
  emit(): void {
    this.advance();
    this.notify();
  }
