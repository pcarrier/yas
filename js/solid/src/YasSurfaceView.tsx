...
export interface YasSurfaceViewProps {
   ...
   */
  resizable?: boolean;
  /** Changing this value measures the pane after layout, without rebuilding
   *  its stream or waiting for ResizeObserver. Useful for fullscreen toggles. */
...
export function YasSurfaceView(props: YasSurfaceViewProps) {
  ...
  // Ancestors can recompute visibility when geometry changes without hiding
  // this pane. Only an actual boolean change should dispose the resize driver
  // and withdraw its size claim.
  const resizable = createMemo(() => props.resizable !== false);
  ...
  // Wait until every ancestor has applied its display/flex changes. Hidden
  // siblings release their size claims above; the visible view then offers
  // its final box. Cancel stale work on another toggle or unmount.
  createEffect(
    on([mounted, resizable, () => props.resizeKey], () => {
      if (!mounted() || !resizable()) return;
