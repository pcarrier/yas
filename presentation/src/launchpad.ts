...
function bestPort<T extends MIDIInput | MIDIOutput>(
  ports: Iterable<T>,
): T | null {
  return (
    [...ports]
      .filter(
        (port) => port.state === "connected" && isLaunchpad(port.name ?? ""),
      )
      .sort(
        (left, right) =>
          portScore(right.name ?? "") - portScore(left.name ?? ""),
      )[0] ?? null
  );
}
...
export class LaunchpadMiniMk3 {
  ...
  get connected(): boolean {
    return (
      this.input?.state === "connected" && this.output?.state === "connected"
    );
  }
  ...
  private paint(): void {
    ...
    for (let index = 0; index < SLIDE_PAD_NOTES.length; index += 1) {
      const color =
        index < state.slideMidiColors.length
          ? (state.slideMidiColors[index] ?? DIM)
          : OFF;
      this.note(
        SLIDE_PAD_NOTES[index],
        index === state.slideIndex ? WHITE : color,
      );
    }
    for (let index = 0; index < SECTION_PAD_NOTES.length; index += 1) {
      const start = state.sectionStarts[index];
      const nextStart =
        state.sectionStarts[index + 1] ?? Number.POSITIVE_INFINITY;
      const active = state.slideIndex >= start && state.slideIndex < nextStart;
      this.note(
        SECTION_PAD_NOTES[index],
        active ? WHITE : (state.sectionMidiColors[index] ?? DIM),
      );
    }
  }
