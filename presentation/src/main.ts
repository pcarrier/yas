...
function positionExplanation(clientX: number, clientY: number): void {
  ...
  const top = below + rect.height <= window.innerHeight - edge
    ? below
...
function annotateLines(slideId: string, slideTitle: string): void {
  content.querySelectorAll<HTMLElement>("pre > code:not(.language-mermaid)").forEach((code) => {
    const lines = (code.textContent ?? "").replace(/\n$/, "").split("\n");
    code.replaceChildren(...lines.flatMap((line, lineIndex) => {
      const span = document.createElement("span");
      span.className = "code-line";
      span.textContent = line || " ";
      return lineIndex < lines.length - 1 ? [span, document.createTextNode("\n")] : [span];
    }));
  });
...
function render(): void {
  ...
  const diagrams = [...content.querySelectorAll<HTMLElement>("pre > code.language-mermaid")].map((code) => {
  ...
  if (diagrams.length > 0) {
    ...
    queueMicrotask(() => {
      ...
      void mermaid.run({ nodes: diagrams }).catch((error: unknown) => {
        if (revision === renderRevision) console.error("Mermaid render failed", error);
      });
    });
  }
  syncController();
...
function showControls(): void {
  ...
  controlsTimer = window.setTimeout(() => deck.classList.remove("controls-visible"), 1800);
}
...
root.addEventListener("click", (event) => {
  ...
  else if (!overviewOpen && !helpOpen) go(event.clientX < window.innerWidth / 2 ? index - 1 : index + 1);
});
...
root.addEventListener("pointerover", (event) => {
  const target = (event.target as HTMLElement).closest<HTMLElement>("[data-explanation]");
...
root.addEventListener("pointerout", (event) => {
  ...
  const nextLine = next instanceof Element ? next.closest("[data-explanation]") : null;
...
document.addEventListener("keydown", (event) => {
  ...
  if (["ArrowRight", "PageDown", "Space", "Enter"].includes(code)) go(index + 1);
