...
mutually pinned Ed25519 identities and encrypts the YAS session end to end
with TLS 1.3 and ephemeral X25519. On each endpoint, set
...
| `YAS_UPLINK_IDENTITY`                            | unset                              | Ed25519 private seed as 43-character base64url; producer `--identity` or consumer URI `identity` overrides it                                                                |
| `YAS_UPLINK_CLIENT_KEYS`                         | unset                              | Producer allowlist of comma-separated Ed25519 public keys (43-character base64url each); equivalent to repeatable `--allow-client`                                           |
