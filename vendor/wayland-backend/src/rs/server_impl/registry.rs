        ...
        if !global.disabled {
            ...
            for registry in self.known_registries.iter().cloned() {
                if let Ok(client) = clients.get_client_mut(registry.client_id.clone()) {
                    // A filtered global was never advertised to a client that
                    // cannot view it -- all three paths that send
                    // wl_registry.global consult can_view first.  Withdrawing
                    // it has to be just as private: a global_remove naming a
                    // global the client has no record of both leaks the
                    // existence of another client's resource and breaks any
                    // client that keys a map by registry name.
