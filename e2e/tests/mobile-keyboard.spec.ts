...
test("the iPadOS shortcut bar counts as an open keyboard", async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    if (!owner) throw new Error("no focused workspace owner");
    ...
    // Keep the synthetic active target first in the owner's keyboard-target
    // order if the settled workspace runs a same-selection focus pass.
    owner.prepend(input);
    input.focus();
  });
