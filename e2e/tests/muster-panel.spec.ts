...
test.describe("muster panel", () => {
  ...
  test("nests a stack's units under the instance that expanded them", async ({
    ...
    await expect(
      page.getByText(/terminal [0-9a-f]{16}$/).first(),
    ).toBeVisible();
    ...
    const retained = memberCard.getByRole("button", {
      name: /^Open retained terminal [0-9a-f]{16}, exit /,
    });
    await expect(retained).toBeVisible();
    await expect(retained).toContainText(/[0-9a-f]{16}\s*· exit /);
    await expect(retained).toHaveAttribute(
      "data-muster-terminal",
      /^[0-9a-f]{16}$/,
    );
  ...
  test("terminal chips drag into the focused view", async ({ page }) => {
    ...
    await expect(terminal).toContainText(/terminal [0-9a-f]{16}$/);
