 ...
 *
 * XDG Desktop and systemd are tabs of one server's Manage panel, and each exists
...
const MODULE = path.resolve(
  __dirname,
  "../../extensions/dist/xdg-desktop.wasm",
);
const XDG_DESKTOP_TAB = '[data-connection-tab="xdg-desktop"]';
...
test("installing an extension adds its tab, removing it takes it away", async ({
  ...
  if (!fs.existsSync(MODULE)) {
    test.skip(
      true,
      `no xdg-desktop extension at ${MODULE} (run bin/extensions)`,
    );
  }
   ...
   *  called `xdg-desktop`, and a name that resolves to the wrong one fails in a
  ...
  /** Remove every extension, so "no XDG Desktop tab" is a fact rather than a hope.
  ...
  await expect(page.locator(XDG_DESKTOP_TAB)).toHaveCount(0);
  ...
  try {
    ...
    const installed = extensionSelector(
      yas("ext", "run", "--persist", "xdg-desktop", MODULE),
    );
    expect(installed).toMatch(/^id:[0-9a-f]{16}$/);
    await expect(page.locator(XDG_DESKTOP_TAB)).toHaveCount(1, {
      timeout: 15_000,
    });
    await expect(page.locator(XDG_DESKTOP_TAB)).toHaveText("XDG Desktop");
