 ...
 *
 * HTML5 drag-and-drop never fires from touch, so `startPaneTouchDrag` bridges
 * pointer events to the same DragEvents the pane and park targets listen for.
 *
...
test.describe("Grip drag with a finger", () => {
  ...
  test("dragging over content leaves the status-bar toolbar fixed", async ({
    page,
  }) => {
    await authenticate(page);
    await newTerminal(page);
