...
/**
 * PaneTools lives in the status bar: every focused pane kind gets one stable
...
/** Focus the main view and return the status bar's pane grip. */
...
test.describe("Pane multitool on a main-view terminal", () => {
  test("the status bar exposes the focused pane actions", async ({ page }) => {
  ...
  test("the status-bar actions follow focus to another pane", async ({
    page,
  }) => {
    await authenticate(page);
    await newTerminal(page);
