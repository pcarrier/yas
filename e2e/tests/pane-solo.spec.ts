 ...
 * unmounted. Reachable from the focused-pane toolbar and from Ctrl+B z.
 *
...
test.describe("Pane solo", () => {
  ...
  test("the multitool's solo segment fills the workspace and restores", async ({
    ...
    await first.locator(".yas-scroll-surface").click({ force: true });
    const solo = page.getByRole("button", { name: /Zoom this pane/ }).first();
    ...
    await first.locator(".yas-scroll-surface").click({ force: true });
    const unsolo = page
      .getByRole("button", { name: /Restore all panes/ })
      .first();
  ...
  test("Ctrl+B z toggles solo on the focused pane", async ({ page }) => {
    await authenticate(page);
    await newTerminal(page);
    await newTerminal(page);
    await twoPanes(page);
  ...
  test("a single-pane layout offers no solo", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Zoom this pane/ }),
    ).toHaveCount(0);
  });
});
