...
impl Muster {
    ...
    fn stop(&mut self, client: &mut Client, name: &str, cause: Cause, hold: bool) {
        // A successful oneshot has already stopped running. Its exited result
        // is what makes it ready, so withdrawing that result would only send
        // healthy consumers into Waiting without stopping a process.
        if self.units.get(name).is_some_and(Unit::is_completed_oneshot) {
            return;
        }
        self.stop_dependents(client, name);
        if let Some(unit) = self.units.get_mut(name) {
    ...
    fn panel_command(&mut self, client: &mut Client, verb: &str, name: &str, now: u64) {
        match verb {
            ...
            "reload" if !name.is_empty() => {
                let _ = self.reload_unit(client, name);
            }
