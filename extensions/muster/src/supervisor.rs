...
/// A longer base than the server and `@xdg-desktop` use: every retry creates a new
...
impl Unit {
    ...
    ///
    /// The policy is obeyed literally. `@xdg-desktop` refuses to retry exit 0 even
    ...
    ///
    /// A successful oneshot has no live process, and its completed result is
    /// the readiness it provides. Treating a stop command as withdrawal of
    /// that result would needlessly stop every consumer and leave them waiting.
    pub fn is_completed_oneshot(&self) -> bool {
        self.file.unit_type == crate::config::UnitType::Oneshot
            && self.phase == Phase::Exited
            && self.pty.is_none()
    }
    ...
    /// Forget retained-run metadata when the Terminal catalogue says the PTY
    /// itself is gone. Missing terminals must not continue consuming `keep`
    /// slots: retention is a promise about addressable PTYs, not just history.
    pub fn forget_run(&mut self, pty: u64) -> bool {
        let before = self.runs.len();
        self.runs.retain(|run| run.pty != pty);
        self.runs.len() != before
    }

    /// Reconcile retained metadata with an authoritative Terminal snapshot.
    /// Keep the newest copy if older buggy supervisors recorded a PTY more
    /// than once.
    pub fn retain_present_runs(&mut self, present: &BTreeSet<u64>) -> bool {
        let before = self.runs.len();
        let mut seen = BTreeSet::new();
        self.runs
            .retain(|run| present.contains(&run.pty) && seen.insert(run.pty));
        self.runs.len() != before
    }
...
mod tests {
    ...
    fn a_oneshot_that_succeeds_counts_as_ready() {
        ...
        assert!(
            unit.is_completed_oneshot(),
            "there is no process for stop to affect"
        );
    }

    #[test]
    fn an_in_progress_oneshot_can_still_be_stopped() {
        let mut unit = unit_from(r#"{"command":["a"],"type":"oneshot"}"#);
        unit.phase = Phase::Activating;
        unit.pty = Some(7);
        assert!(!unit.is_completed_oneshot());
    }
    ...
    #[test]
    fn a_removed_terminal_no_longer_consumes_a_retention_slot() {
        let mut unit = unit_from(r#"{"command":["a"],"keep":2}"#);
        for pty in [7, 8] {
            unit.pty = Some(pty);
            unit.seq = pty + 1;
            unit.phase = Phase::Running;
            assert!(unit.note_exit(1, 1000, 0).is_empty());
        }

        assert!(unit.forget_run(7));
        assert!(!unit.forget_run(7), "forgetting a run is idempotent");
        assert_eq!(unit.runs.iter().map(|run| run.pty).collect::<Vec<_>>(), [8]);

        unit.pty = Some(9);
        unit.seq = 10;
        unit.phase = Phase::Running;
        assert!(
            unit.note_exit(1, 1001, 0).is_empty(),
            "the replacement PTY fills the freed slot instead of being reaped"
        );
        assert_eq!(
            unit.runs.iter().map(|run| run.pty).collect::<Vec<_>>(),
            [9, 8]
        );
    }

    #[test]
    fn a_terminal_snapshot_prunes_missing_and_duplicate_run_metadata() {
        let mut unit = unit_from(r#"{"command":["a"],"keep":3}"#);
        for pty in [8, 8, 7] {
            unit.runs.push(Run {
                pty,
                seq: pty,
        ...
        assert!(unit.retain_present_runs(&BTreeSet::from([8, 9])));
        assert_eq!(unit.runs.iter().map(|run| run.pty).collect::<Vec<_>>(), [8]);
    }
