...
explicit `stop` of an in-progress run still takes the dependency tree down
immediately. Stopping a completed `oneshot` is a no-op: it has no process, and
its successful result remains ready for dependents.
...
`restart NAME`, `reload NAME`, `rewatch`, `resync`. A name is a unit or an
instance.
