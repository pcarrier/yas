...
```bash
yas ext run --persist --restart always xdg-desktop extensions/dist/xdg-desktop.wasm
yas @xdg-desktop list
yas @xdg-desktop enable legcord
yas @xdg-desktop status legcord
yas @xdg-desktop disable legcord

# This session only, leaving intent alone
yas @xdg-desktop start legcord
yas @xdg-desktop stop legcord

# Off the list entirely
yas @xdg-desktop forget legcord
```
...
`yas.xdg-desktop.v1` publishes the whole state as JSON (one `{"type":"state"}`
...
`resync`, each followed by a desktop-entry id. `js/ui/src/xdgDesktop.ts` mirrors it, and the
**XDG Desktop** tab of an expanded remote is what a viewer sees.
...
`cargo test --manifest-path extensions/Cargo.toml -p yas-ext-xdg-desktop` covers
