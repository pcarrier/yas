...
> {
    ...
    // a process-wide provider before web-transport-quinn's builder asks for
    // one (both crypto features may be unified elsewhere in the workspace).
...
async fn bridge_composite_web_transport(
    ...
    tokio::select! {
        _ = reliable_down => {}
        _ = reliable_up => {}
        _ = async { tokio::join!(datagram_down, datagram_up); } => {
            // Losing both datagram directions does not invalidate the main
            // link. Park until the authoritative stream completes.
