...
impl Compositor {
    ...
    fn dispatch_pointer_motion(&mut self, surface_id: u16, x: f64, y: f64, time_ms: u32) {
        ...
        if let Some((proto_id, wl_surface, lx, ly)) = target_wl {
            ...
            // Chromium video controls can enter an already-idle player with
            // its old `cursor: none` still selected and fail to publish the
            // corresponding visible cursor after motion.  Absolute motion is
            // itself proof that an idle hide should end.  Preserve hidden only
            // for a real Wayland pointer lock, where it is intentional.
            if matches!(self.last_cursor.get(&surface_id), Some(CursorImage::Hidden))
