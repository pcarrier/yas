...
impl Dispatch<wl_data_source::WlDataSource, ()> for App {
    fn event(
        ...
        if let wl_data_source::Event::Send { mime_type, fd } = event {
            std::thread::sleep(state.source_send_delay);
...
fn answer_source_send(queue: &mut EventQueue<App>, app: &mut App) {
    let deadline = Instant::now() + Duration::from_secs(2);
    while app.source_sends.is_empty() {
        assert!(
            Instant::now() < deadline,
            "clipboard owner received no send"
        );
        // A roundtrip can finish before the compositor handles ClipboardGet
        // or the other client's receive. Keep dispatching until Send arrives.
        queue.roundtrip(app).expect("owner answers clipboard send");
    }
}
...
fn a_wayland_clients_image_selection_splices_directly_to_another_client() {
    ...
    let mut owner = App {
        source_send_delay: Duration::from_millis(25),
        ..App::default()
    };
    owner_queue
        .roundtrip(&mut owner)
    ...
    // the fetch waits for readable data instead of spuriously returning none.
    ...
    assert_eq!(
        response
            ...
            .expect("clipboard get reply")
            .expect("clipboard get content"),
        PNG,
    );
    owner.source_sends.clear();
