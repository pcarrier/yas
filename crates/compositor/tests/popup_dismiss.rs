...
fn popup_done_publishes_the_uncovered_toplevel_without_another_commit() {
    ...
    // The command queue and the Wayland socket are independent inputs.  If
    // the first dispatch observes the sync request just before the wake, its
    // roundtrip can complete before the already-queued pointer commands run.
    // A second roundtrip then orders after that command-loop pass and observes
    // popup_done without adding an arbitrary sleep.
    if !app.popup_done {
        queue
            .roundtrip(&mut app)
            .expect("post-command dismiss roundtrip");
    }
