...
fn windows_uri_path(path: &str) -> String {
    ...
    // Canonical Windows paths use the extended-length namespace. It is a
    // filesystem detail, not part of an LSP file URI.
    let path = path
        .strip_prefix("//?/UNC/")
        .map(|unc| format!("//{unc}"))
        .unwrap_or_else(|| path.strip_prefix("//?/").unwrap_or(&path).to_owned());
    if let Some(unc) = path.strip_prefix("//") {
        // The server is the URI authority: file://server/share/file.
        unc.to_owned()
...
#[cfg(any(windows, test))]
fn windows_path_from_uri(authority: &str, path: &str) -> PathBuf {
    let path = path.replace('/', "\\");
    if !authority.is_empty() && !authority.eq_ignore_ascii_case("localhost") {
        return PathBuf::from(format!(r"\\?\UNC\{authority}{path}"));
    }
    let path = path
        .strip_prefix('\\')
        .filter(|p| p.as_bytes().get(1) == Some(&b':'))
        .unwrap_or(&path);
    // The backend's roots and document keys are canonical Windows paths.
    // Restore that namespace lexically, including for not-yet-created files.
...
pub fn path_to_uri(path: &Path) -> String {
    ...
    #[cfg(windows)]
    let text = windows_uri_path(&text);
...
pub fn uri_to_path(uri: &str) -> Option<PathBuf> {
    ...
    #[cfg(windows)]
    {
        Some(windows_path_from_uri(&rest[..slash], &text))
    }
    #[cfg(not(windows))]
...
mod tests {
    ...
    #[test]
    fn windows_drive_and_unc_uri_paths_preserve_canonical_identity() {
        for (plain, canonical, authority, path) in [
            (
                r"C:\work\a b\λ.rs",
                r"\\?\C:\work\a b\λ.rs",
                "",
            ...
            let uri_path = format!("{authority}{path}");
            assert_eq!(windows_uri_path(plain), uri_path);
            assert_eq!(windows_uri_path(canonical), uri_path);
            assert_eq!(
                windows_path_from_uri(authority, path),
                PathBuf::from(canonical)
            );
        }
        assert_eq!(
            windows_path_from_uri("localhost", "/C:/work/new.rs"),
    ...
    #[cfg(windows)]
    #[test]
    fn canonical_windows_file_uri_roundtrip() {
        let path = Path::new(r"\\?\C:\work\a b\λ.rs");
        let uri = path_to_uri(path);
        assert_eq!(uri, "file:///C:/work/a%20b/%CE%BB.rs");
        assert_eq!(uri_to_path(&uri).as_deref(), Some(path));
        let path = Path::new(r"\\?\UNC\server\share\new.rs");
        let uri = path_to_uri(path);
        assert_eq!(uri, "file://server/share/new.rs");
        assert_eq!(uri_to_path(&uri).as_deref(), Some(path));
    }
}
