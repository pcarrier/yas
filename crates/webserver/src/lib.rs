...
fn if_none_match_matches(if_none_match: Option<&[u8]>, etag: &str) -> bool {
    let Some(value) = if_none_match.and_then(|value| std::str::from_utf8(value).ok()) else {
        return false;
    };
    ...
    if accepts_br(accept_encoding) {
        (
            [
                ...
                (header::VARY, "Accept-Encoding".to_owned()),
            ],
            html_br,
        )
            .into_response()
...
mod tests {
    ...
    #[tokio::test]
    async fn html_response_accepts_etag_lists_and_weak_validators() {
        let etag = html_etag(b"hello");
        let condition = format!("\"other\", W/{etag}");
        let resp = html_response(b"hello", &etag, Some(condition.as_bytes()), None);
        assert_eq!(resp.status(), StatusCode::NOT_MODIFIED);
    }
