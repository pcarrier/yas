...
async fn control_once(
    ...
    let identity: DefinitionIdentity = client
        .request_typed(
            family::EXTENSION,
            wire::request_kind::CONTROL,
            &Control {
...
impl ModuleObject {
    ...
    async fn fetch(url: &str) -> Result<Self, String> {
        ...
        if response
            .content_length()
            .is_some_and(|length| length > wire::MAX_OBJECT_BYTES)
        {
            return Err(format!(
                "{url} declares more than {} bytes",
                wire::MAX_OBJECT_BYTES
            ));
        }
        ...
        {
            if bytes.len() as u64 + chunk.len() as u64 > wire::MAX_OBJECT_BYTES {
                return Err(format!(
                    "{url} exceeds the {}-byte extension limit",
                    wire::MAX_OBJECT_BYTES
                ));
            }
