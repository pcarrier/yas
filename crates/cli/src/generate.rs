...
fn yas_edge_cmd(name: &'static str) -> Command {
    Command::new(name)
        ...
        .about("YAS browser edge over WebSocket and optional WebTransport")
        .long_about(
            "yas-edge serves the browser UI, authenticates browser transports, and \
             adapts one authenticated WebSocket or WebTransport session to one fixed home YAS socket. It does \
        ...
        .after_help(
            "ENVIRONMENT:\n    \
             ...
             YAS_WEBTRANSPORT Enable native QUIC datagrams when set to 1\n    \
