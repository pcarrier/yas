...
impl Deploy {
    pub fn preserve_argv(&self) -> bool {
        self.extensions.0.iter().any(|extension| {
            extension.tag == crate::schema::extension::DEPLOY_PRESERVE_ARGV_TAG as u16
        })
    }
    ...
    fn validate(&self) -> Result<()> {
        ...
        if let Some(extension) = self.extensions.0.iter().find(|extension| {
            extension.tag == crate::schema::extension::DEPLOY_PRESERVE_ARGV_TAG as u16
        }) && (matches!(expected, (0, 0, 0))
            || !extension.value.is_empty()
            || !self.argv.is_empty())
        {
            return Err(Error::Invalid("Extension preserve-argv deploy extension"));
        }
        self.runtime_limits.validate()?;
        reject_unknown_required(
            &self.extensions,
            &[crate::schema::extension::DEPLOY_PRESERVE_ARGV_TAG as u16],
        )
    }
}
    ...
    fn object_and_definition_requests_round_trip() {
        ...
        let preserve = Deploy {
            ...
            extensions: Extensions(vec![crate::codec::Extension {
                tag: crate::schema::extension::DEPLOY_PRESERVE_ARGV_TAG as u16,
                required: true,
                value: Vec::new(),
            }]),
        };
        assert!(preserve.preserve_argv());
        round_trip(preserve.clone());
        let mut invalid_preserve = preserve;
        invalid_preserve.argv.push(b"replacement".to_vec());
        assert!(invalid_preserve.encode().is_err());
        let invalid = Deploy {
            operation_id: [3; 16],
            expected_extension_handle: 5,
            expected_generation: 0,
