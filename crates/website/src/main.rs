...
static SKILL_ETAG: LazyLock<String> = LazyLock::new(|| content_etag(SKILL.as_bytes()));
...
fn script_response(installer: Installer, request_headers: &HeaderMap) -> Response {
    let (body, content_type, etag) = match installer {
        Installer::Shell => (
            INSTALL_SH,
            "text/x-shellscript; charset=utf-8",
            &*INSTALL_SH_ETAG,
        ),
        Installer::PowerShell => (INSTALL_PS1, "text/plain; charset=utf-8", &*INSTALL_PS1_ETAG),
    };
    static_response(
        body.as_bytes(),
        content_type,
...
async fn root(headers: HeaderMap) -> Response {
    let mut response = match installer_for_request(&headers) {
        Some(installer) => script_response(installer, &headers),
        None => asset_response("index.html", &headers),
    };
    response
        .headers_mut()
...
async fn install_sh(headers: HeaderMap) -> Response {
    script_response(Installer::Shell, &headers)
}

async fn install_ps1(headers: HeaderMap) -> Response {
    script_response(Installer::PowerShell, &headers)
}

async fn skill(headers: HeaderMap) -> Response {
    static_response(
        SKILL.as_bytes(),
...
async fn static_asset(headers: HeaderMap, uri: Uri) -> Response {
    let path = uri.path().trim_start_matches('/');
    let path = match path {
        "s" | "s/" => "s/index.html",
        path => path,
    };
    asset_response(path, &headers)
}

fn asset_response(path: &str, request_headers: &HeaderMap) -> Response {
    let Some((_, bytes, etag)) = embedded::ASSETS.iter().find(|(name, _, _)| *name == path) else {
        return (StatusCode::NOT_FOUND, "not found").into_response();
    };
    ...
    static_response(bytes, content_type, cache, etag, request_headers)
}
...
mod tests {
    ...
    #[test]
    fn serves_assets_with_cache_headers_and_etags() {
        let html = asset_response("index.html", &HeaderMap::new());
        ...
        let asset = embedded::ASSETS
            .iter()
            .find(|(path, _, _)| path.starts_with("assets/"))
            .expect("built website has a fingerprinted asset");
        let response = asset_response(asset.0, &HeaderMap::new());
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(
            response.headers()[header::CACHE_CONTROL],
            "public, max-age=31536000, immutable"
        );
        assert_eq!(response.headers()[header::ETAG], asset.2);
    }

    #[test]
    fn returns_not_modified_for_matching_asset_etag() {
        let first = asset_response("index.html", &HeaderMap::new());
        let etag = first.headers()[header::ETAG].clone();
        let mut headers = HeaderMap::new();
        headers.insert(header::IF_NONE_MATCH, etag.clone());

        let response = asset_response("index.html", &headers);
    ...
    #[tokio::test]
    async fn static_documents_have_etags_and_root_varies_by_representation() {
        let first = install_sh(HeaderMap::new()).await;
        let etag = first.headers()[header::ETAG].clone();
        let mut headers = HeaderMap::new();
        headers.insert(header::IF_NONE_MATCH, etag.clone());
        let cached = install_sh(headers).await;
