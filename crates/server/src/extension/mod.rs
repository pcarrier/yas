...
impl ExtensionService {
    ...
    /// Load the arguments for one exact persistent definition. Registry
    /// updates use this because public State intentionally omits raw argv.
    pub(crate) async fn native_definition_arguments(
        &self,
        extension_handle: u64,
        generation: u64,
        definition_revision: u64,
    ) -> Result<Vec<Vec<u8>>, NativeMutationFailure> {
        let definition = {
            let inner = self.inner.lock().await;
            let definition = inner
                .definitions
        ...
        self.definition_arguments(&definition)
            .await
            .map_err(|error| NativeMutationFailure::Internal(error.to_string()))
    }
...
async fn drive_attempt(
    ...
    // Preparation has instantiated the runtime and the native handler is ready.
    // Publish RUNNING before releasing the guest's start latch: once guest code
    // can issue authenticated requests, operations such as REGISTER_COMMAND
    // must observe the attempt as running.
