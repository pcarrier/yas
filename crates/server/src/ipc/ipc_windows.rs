...
async fn bind_replacing_existing(pipe_name: &str) -> io::Result<NamedPipeServer> {
    ...
    for attempt in 0..=REPLACE_ATTEMPTS {
        ...
        if !shutdown_requested && let Ok(previous) = ClientOptions::new().open(pipe_name) {
            if request_shutdown(previous).await.is_ok() {
...
async fn request_shutdown(
    mut previous: tokio::net::windows::named_pipe::NamedPipeClient,
) -> io::Result<()> {
    ...
    let pre_hello_codec = FrameCodec::pre_hello();
    let session_codec = FrameCodec::new(pre_hello_codec.limits(), []).map_err(io::Error::other)?;
    let hello = pre_hello_codec
        .encode_stream(&Frame {
...
mod tests {
    ...
    async fn bind_replaces_an_existing_server() {
        ...
        let previous_task = tokio::spawn(async move {
            ...
            previous.read_to_end(&mut bytes).await.unwrap();
            assert!(bytes.starts_with(&yas_wire::PREFACE));
            let bytes = &bytes[yas_wire::PREFACE.len()..];
            let pre_hello_codec = FrameCodec::pre_hello();
            let (hello, consumed) = pre_hello_codec.decode_stream(bytes).unwrap();
            assert_eq!(
                hello.header,
                FrameHeader::request(family::CORE, yas_wire::core::request_kind::HELLO, 1)
            );
            let session_codec = FrameCodec::new(pre_hello_codec.limits(), []).unwrap();
            let (shutdown, shutdown_bytes) =
                session_codec.decode_stream(&bytes[consumed..]).unwrap();
            assert_eq!(shutdown.header.kind, yas_wire::core::request_kind::SHUTDOWN);
            assert_eq!(consumed + shutdown_bytes, bytes.len());
        });
