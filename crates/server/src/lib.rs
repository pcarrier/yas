...
fn clamp_cursor_rect((x, y, width, height): (i32, i32, i32, i32)) -> Option<(i16, i16, i16, i16)> {
    // A caret may be a zero-width vertical line. Keep its position and line
    // height, using the minimum positive width required by the wire format.
    // A 0x0 rectangle (e.g. Firefox acquiring address-bar focus) still has no
    // useful line height to anchor the IME against.
    if width < 0 || height <= 0 {
        return None;
    }
...
mod cursor_rect_tests {
    ...
    #[test]
    fn a_zero_width_caret_keeps_its_position_and_line_height() {
        assert_eq!(clamp_cursor_rect((10, 20, 0, 12)), Some((10, 20, 1, 12)));
    }

    #[test]
    fn empty_or_negative_caret_extents_are_omitted() {
        ...
        assert_eq!(clamp_cursor_rect((10, 20, 1, -1)), None);
    }
...
fn surface_credit_limit_bytes(client: &ClientState, next_frame_bytes: usize) -> usize {
    let window_secs = path_rtt_ms(client).max(0.0) / 1_000.0;
    ...
    if client.surface_goodput_sampled {
        // Keep no deliberate queue above the measured bandwidth-delay
        // product. Surface video is reliable and ordered, so speculative
        // probe bytes turn directly into latency under load. One encoded
        // frame is indivisible; `next` is therefore the unavoidable floor,
        // and a blocked successor lowers effective cadence and drives the
        // ordinary quantizer adaptation.
        return measured.max(next);
    }
...
mod tests {
    ...
    #[test]
    fn measured_surface_credit_has_no_queue_above_the_bandwidth_delay_window() {
        let (mut client, _rx) = test_client_with_capacity(64);
        client.rtt_ms = 50.0;
        client.min_rtt_ms = 50.0;
